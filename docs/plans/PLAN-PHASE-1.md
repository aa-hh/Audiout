...
(`AudioutCore/`, `dev/audiocap/`, `dev/owntone/`).
...
(`AudioutCore/Sources/AudioutCore/OutputBackend.swift:31`),
...
bundle).
- (a, **recommended**) **New SwiftPM executable target** `AudioutApp`
  in `AudioutCore/Package.swift`, run headlessly as an `.accessory`
  ...
  one `swift build`, links `AudioutCore` directly, no `.xcodeproj` to
...
`.macOS(.v13)` (`AudioutCore/Package.swift:6`, `dev/audiocap/Package.swift:9`).
...
- (a, **recommended**) **A versioned JSON file** at
  `~/Library/Application Support/Audiout/groups.json`, loaded/saved via
...
- Recommendation: (a). Keep the store in `AudioutCore` (a `GroupStore`
...
- (a, **recommended**) In a UI-agnostic `GroupController`/mixer-model type in
  `AudioutCore`, unit-tested (proportional scaling, clamp-at-100,
...
### AudioutCore (UI-agnostic, testable)

**T-C1 — Real `OwnToneBackend`: JSON API control + state**
- files: `AudioutCore/Sources/AudioutCore/OwnToneBackend.swift`
...
- files: NEW `AudioutCore/Sources/AudioutCore/CaptureCoordinator.swift`
...
- files: NEW `AudioutApp/` executable target (Q1(a)) added to
  `AudioutCore/Package.swift:14` (targets/products), NEW
...
- HOT FILE: `AudioutCore/Package.swift` (shared with T-C* only via
  target additions — see waves).

**T-U2 — Menu: device + group rows, sliders/mute/solo, in-place expansion**
- files: NEW `AudioutApp/Menu/` (`MenuController.swift`,
...
- files: NEW `AudioutApp/Menu/GroupEditorView.swift`, edits
  `AudioutApp/Menu/MenuController.swift` (swap-in-place editor mode)
...
- files: NEW `AudioutApp/Window/` (`MixerWindowController.swift`,
...
- files: `SPEC.md` (§5 Phase-1 status, §9 note the menu-mutation verdict + the
  ...
  app), `AudioutCore/MEMORY.md`-index pointer if warranted
...
- `AudioutCore/Package.swift` — touched by T-U1 (add app target) and
  potentially T-C2/T-C3 (new files need target membership under the existing
  `AudioutCore` target — actually only new *targets* touch it; new
  *files* in the existing target do NOT edit Package.swift). Net: **only T-U1
  ...
  edit behind T-U1.
- `AudioutApp/Menu/MenuController.swift` — T-U2 creates it, T-U3 edits
...
- **New unit tests (in `AudioutCoreTests`, keep the existing 10 green):**
  ...
  any unit test (all model/backend logic lives in `AudioutCore`).
...
- **Package.swift:** one edit (T-U1) adds the `AudioutApp` executable
...
- **Q6 Master math:** UI-agnostic GroupController in AudioutCore, unit-tested.
...
- **T-U1 ✅** app skeleton: AudioutApp executable target, real .app bundle (LSUIElement, ad-hoc signed, launches/quits clean), status item, mock-first.
- **T-U2 ✅** the menu: pure-AppKit dropdown, LIVE in-place group expansion (no fallback needed), shared DeviceRowView, quick-create action. Menu factored into AudioutMenuUI library target (testable). 45 tests + 28-check harness.
