...
- [Sources/AirPlayControllerCore/](Sources/AirPlayControllerCore/) — the library: `Device`,
  `OutputBackend`/`BackendEvent`, `MockBackend`, `OwnToneBackend` + `makeBackend`.
...
| Type | Role |
|---|---|
| `Device` | Value-type snapshot of one AirPlay output (identity, kind, volume, mute/solo/selected state). |
...
| `OwnToneBackend` | Stub for the real backend (OwnTone JSON API + AirPlay-2 sender). Every method is `unimplemented()`/asserts — not yet wired up. |
...
| Area | Status |
|---|---|
| `OwnToneBackend` | Stub only — `start()`/`setVolume`/`setMuted`/`setSoloed`/`setOutputSet` all call `unimplemented()` (assertion failure). Real implementation is Phase 0d/0f → Phase 1 work per SPEC.md §5, blocked on physical speaker access (see project memory: speakers currently unavailable, mock rig is primary dev target). OwnTone pipe-input rate/autostart findings are captured in `dev/notes/0f-pipe-brief.md` at the repo root, not in this package. |
| Per-app routing (`AppRoutingController`/`AppRouteStore`) | UI + model + persistence COMPLETE (SPEC.md §9, PLAN-POPOVER-ROUTING.md T-1..T-8/T-11, all landed, 148 tests green). Routes persist and render but move no audio yet — wired against `MockBackend` only; blocked on the native engine gaining per-app capture streams (`CaptureCoordinator` is still a single global tap today). |
