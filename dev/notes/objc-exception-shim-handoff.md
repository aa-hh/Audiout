...
`AudioutCore/*` paths) for the session owning the per-app-routing worktree
...
2. Cherry-pick/merge `3760b2a` directly now. Your worktree predates the
   `AudioutCore` rename, so this commit's paths (`AudioutCore/Sources/...`)
...
`AudioutCore/Sources/ObjCExceptionShim/include/ObjCExceptionShim.h`:
