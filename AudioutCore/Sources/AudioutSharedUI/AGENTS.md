...
Pure UI: controls route through a delegate, never a backend, store, or `GroupController`.
...
- `setContent`'s `defaultSize:` seeds only a content controller's first mount.
...
- Traps, decisions, changelog: [AGENTS-HISTORY.md](AGENTS-HISTORY.md); grep it before debugging.
