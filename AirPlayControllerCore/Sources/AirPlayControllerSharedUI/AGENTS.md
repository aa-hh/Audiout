...
**Keep up to date when:** a row's control set / layout column changes, `PopoverColumnGrid` constants move, the connection-status badge or sublabel rendering changes, a device-row sublabel scheme is revised, a row's selection/removal affordances change, or a new shared row type lands here.
...
- **Stock bezel over custom hover (2026-07-17).** The custom hover-highlight button family that used to live here (`RowHoverButton` / `NeutralHoverButton`, plus the `AddApplicationRowView` row they styled) is **DELETED**. Add/remove is now the host-owned `NSSegmentedControl` ± footer, and the popover header's icon buttons are stock `NSButton`s with `bezelStyle = .smallSquare` — AppKit draws their hover state. Reach for a stock bezel before hand-rolling a hover wash. The device-row selection pill and `AppRowView`'s hover/selection washes are drawn by the rows themselves and are unaffected.
