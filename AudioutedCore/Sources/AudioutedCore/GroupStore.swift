...
public struct GroupStore: Sendable {
    ...
    ///   `Application Support/Audiouted/` under the user's Library.
    ...
    /// `~/Library/Application Support/Audiouted/` — created lazily by
    /// `save(_:)`, not here (this is just a pure path computation).
    public static var defaultDirectory: URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? FileManager.default.temporaryDirectory
        return base.appendingPathComponent("Audiouted", isDirectory: true)
    }
