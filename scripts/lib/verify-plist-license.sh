...
verify_app_plist_license_url() {
  _vapl_actual=""
  _vapl_actual="$(plutil -extract AudioutLicenseServerURL raw -o - "$1/Contents/Info.plist" 2>/dev/null)" && [ "$_vapl_actual" = "$2" ] || { echo "ERROR: Info.plist AudioutLicenseServerURL is '${_vapl_actual:-<missing>}', expected '$2' — the shipped app validates no keys and never updates" >&2; return 1; }
}
