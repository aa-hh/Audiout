...
| `AlignmentProbeSession` | One phone-measured probe run (license-clean): plays the `.probe` tick and alternates which speaker is audible — bed-only preamble with both up, then REF/TGT blocks split by both-muted gaps — so a single microphone stream carries the two speakers in strict alternation and no clock is ever compared. TRAP: the pattern constants and the FIRST-BLOCK-IS-REF rule are a contract with the phone's analyzer, not tuning knobs, and the tick injector's preamble must cover the session's — a tick heard from both speakers at once merges into the analyzer's first block. Every exit path (finish, cancel, timeout, target vanishing, deinit) restores the mutes the run took and ends the tick. Mutes go per device even when the reference is "Main Out", because the target is usually a Main Out member itself. |
