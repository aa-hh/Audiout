...
/// The "New Scene" sheet (design revamp, SPEC.md §9 "Group setup"): a standard
...
/// - a "New Scene" title line (design critique: the sheet used to open as an
...
public final class GroupCreationSheetController: NSViewController {
    ...
    private let titleLabel = NSTextField(labelWithString: "New Scene")
    ...
    private func commit() {
        ...
        let name = trimmed.isEmpty ? "New Scene" : trimmed
    ...
    /// The sheet's title line ("New Scene"). `loadViewIfNeeded()` first since a
