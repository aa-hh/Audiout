...
//
// `@convention(c)` function pointers cannot capture Swift state. Under XCTest
// that was papered over by recording into file-scope `var` globals — safe only
// because XCTest gave every test METHOD its own process. swift-testing runs
// tests concurrently inside ONE process, so those globals were a genuine data
// race the moment this file converted. Two different C seams need two different
// answers:
//
//  * `outputs_engine_completion_set(cb, context)` takes a `void *context` that
//    the dispatcher hands straight back to the callback (shims/outputs.h §"Engine
//    completion hook"). The completion hook therefore records into a PER-TEST
//    recorder reached through that context pointer — no shared mutable global at
//    all. This is the preferred fix and it is what the completion path uses.
//
//  * `output_status_cb` — the callback `outputs_callback_add(dev, cb)` registers —
//    is `void (*)(struct output_device *, enum output_device_state)`: NO context
//    parameter, and the device it is handed is deliberately NULL on the teardown
//    path (see `stopRemovesDeadDeviceAndDeliversNull`), so there is nothing to key
//    off either. That one genuinely cannot be made per-instance, so it goes
//    through a lock-guarded file-scope registration slot holding a strong
//    reference to the SAME per-test recorder, installed in `init` and cleared in
//    `deinit` — and the suite nests into `SerializedEngineState` so no two tests
//    can own the slot (or the C registry behind it) at the same time.

/// Per-test recording sink for both C hooks. Locked because the C dispatcher may
/// in principle deliver from whatever thread ran the drain (these tests drive it
/// synchronously, but the box must not be the thing that assumes that).
private final class OutputsCallRecorder: @unchecked Sendable {
    private let lock = NSLock()
    private var status: [(devicePresent: Bool, state: Int32)] = []
    private var completions: [(callbackId: Int32, deviceId: UInt64, state: Int32)] = []

    func recordStatus(devicePresent: Bool, state: Int32) {
        lock.lock(); defer { lock.unlock() }
        status.append((devicePresent: devicePresent, state: state))
    }

    func recordCompletion(callbackId: Int32, deviceId: UInt64, state: Int32) {
        lock.lock(); defer { lock.unlock() }
        completions.append((callbackId: callbackId, deviceId: deviceId, state: state))
    }

    var statusCbCalls: [(devicePresent: Bool, state: Int32)] {
        lock.lock(); defer { lock.unlock() }
        return status
    }

    var completionCalls: [(callbackId: Int32, deviceId: UInt64, state: Int32)] {
        lock.lock(); defer { lock.unlock() }
        return completions
    }
}

/// The one unavoidable file-scope slot: `output_status_cb` has no context
/// parameter, so the context-less C callback has to find the active recorder
/// here. Written only by the suite's `init`/`deinit`, and the suite is
/// serialized, so at most one recorder is ever installed.
private final class ActiveStatusRecorderSlot: @unchecked Sendable {
    private let lock = NSLock()
    private var recorder: OutputsCallRecorder?

    func install(_ recorder: OutputsCallRecorder?) {
        lock.lock(); defer { lock.unlock() }
        self.recorder = recorder
    }

    var current: OutputsCallRecorder? {
        lock.lock(); defer { lock.unlock() }
        return recorder
    }
}

private let activeStatusRecorder = ActiveStatusRecorderSlot()
...
private let recordingStatusCb: @convention(c) (
    ...
    activeStatusRecorder.current?.recordStatus(devicePresent: device != nil, state: state.rawValue)
}
...
) -> Void = { callbackId, deviceId, state, context in
    guard let context else { return }
    Unmanaged<OutputsCallRecorder>.fromOpaque(context)
        .takeUnretainedValue()
        .recordCompletion(callbackId: callbackId, deviceId: deviceId, state: state.rawValue)
}
...
extension SerializedEngineState {
    ...
    // run (it frees heap devices and NULLs the C hooks), and `deinit` is illegal
    // on a struct.
    @Suite final class OutputsDispatcherTests {

        /// Fresh per test — swift-testing instantiates the suite once per `@Test`.
        private let recorder = OutputsCallRecorder()
        ...
        init() {
            ...
            outputs_engine_completion_set(
                recordingCompletionCb,
                Unmanaged.passUnretained(recorder).toOpaque()
            )
            activeStatusRecorder.install(recorder)
        }

        deinit {
            // Tear the C hooks down FIRST. Swift runs `deinit` before releasing the
            // instance's stored properties, so clearing the dispatcher here
            // guarantees the raw context pointer it held can never outlive the
            // `recorder` object it points at.
            outputs_dispatcher_reset()   // NULLs the completion hook *and* its context
            activeStatusRecorder.install(nil)
            drainRegistry()
        }
        ...
        @Test func normalStartOneCallbackCompletes() {
            ...
            #expect(cbId >= 0, "callback_add must yield a valid id")
            ...
            #expect(recorder.completionCalls.count == 0, "delivery must be deferred, not inline")

            outputs_cb_deferred_run()

            // Exactly one completion, carrying the right id/device/state.
            #expect(recorder.completionCalls.count == 1)
            #expect(recorder.completionCalls.first?.callbackId == cbId)
            #expect(recorder.completionCalls.first?.deviceId == deviceId)
            #expect(recorder.completionCalls.first?.state == OUTPUT_STATE_STREAMING.rawValue)

            // Status cb also fired once, with a live device, and device->state updated.
            #expect(recorder.statusCbCalls.count == 1)
            #expect(recorder.statusCbCalls.first?.devicePresent == true)
            #expect(outputs_device_get(deviceId)?.pointee.state.rawValue
                    == OUTPUT_STATE_STREAMING.rawValue)

            // The slot is spent: a second drain delivers nothing (no double-count).
            outputs_cb_deferred_run()
            #expect(recorder.completionCalls.count == 1, "id must fire exactly once")
        }
        ...
        // AirPlayEngineAPITests.outputStateTerminalClassification.)
        @Test func airPlay2StartReportsConnected() {
            ...
            #expect(recorder.completionCalls.count == 1,
                    "CONNECTED is device_start's sole terminal — must fire once")
            #expect(recorder.completionCalls.first?.callbackId == cbId)
            #expect(recorder.completionCalls.first?.deviceId == deviceId)
            #expect(recorder.completionCalls.first?.state == OUTPUT_STATE_CONNECTED.rawValue)
        ...
        @Test func failureAfterConnectIsNoOpOnSpentId() {
            ...
            outputs_cb_deferred_run()
            #expect(recorder.completionCalls.count == 1,
                    "spent id (-1) must not deliver a second completion")
        }

        // MARK: - Error path: FAILED still unblocks (never hangs).

        @Test func errorPathFailedStillCompletes() {
            ...
            #expect(recorder.completionCalls.count == 1, "error path must unblock the waiter")
            #expect(recorder.completionCalls.first?.state == OUTPUT_STATE_FAILED.rawValue)
        }

        // MARK: - Password path: AUTH -> PASSWORD is a terminal report, fires once.

        @Test func passwordPathCompletesOnce() {
            ...
            #expect(recorder.completionCalls.count == 1)
            #expect(recorder.completionCalls.first?.state == OUTPUT_STATE_PASSWORD.rawValue)
        }
        ...
        @Test func authRetryIdHandoffFiresExactlyOnce() {
            ...
            #expect(outputs_callback_get(dev) != nil,
            ...
            #expect(recorder.completionCalls.count == 1,
                    "id survives retry -> exactly one completion")
            #expect(recorder.completionCalls.first?.callbackId == cbId)
        }
        ...
        @Test func replaceOnAddSameDevice() {
        ...
        @Test func zeroCallbackOpRegistersNoWaiter() {
            ...
            #expect(recorder.completionCalls.count == 0, "N=0 op must never produce a completion")
        }

        // MARK: - Defensive no-ops: illegal ids are ignored, never hang/crash.

        @Test func defensiveIllegalIdsAreNoOps() {
            ...
            outputs_cb_deferred_run()
            #expect(recorder.completionCalls.count == 0,
                    "illegal ids must be ignored, not delivered")
        }
        ...
        @Test func stopRemovesDeadDeviceAndDeliversNull() {
            ...
            #expect(recorder.completionCalls.count == 1)
            #expect(recorder.completionCalls.first?.state == OUTPUT_STATE_STOPPED.rawValue)
            // Status cb saw a NULL device (it left the building).
            #expect(recorder.statusCbCalls.first?.devicePresent == false)
            // And it's gone from the registry.
            #expect(outputs_device_get(deviceId) == nil)
        }

        // MARK: - Multiple independent devices in flight: each fires once, no cross-talk.

        @Test func multipleDevicesEachCompleteOnce() {
            ...
            // All distinct slots.
            #expect(Set(cbIds).count == ids.count)
            ...
            #expect(recorder.completionCalls.count == ids.count)
            #expect(Set(recorder.completionCalls.map { $0.deviceId }) == Set(ids))
        }
        ...
        @Test func stopSessionIsLiveFalseWhenSessionIsNull() {
            let dev = makeDevice(id: 0xCAFE, session: false)
            #expect(!AirPlayEngine.stopSessionIsLive(dev))
        }

        @Test func stopSessionIsLiveTrueWhenSessionIsSet() {
