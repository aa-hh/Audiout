...
one surface (`AppSurfaceController` in `AudiouterPopoverUI`). `MixerWindowController` owns NO window;
...
- **The screen's WIDTH is not the surface's to choose either — this pane's
  ...
  window) and why the column cap is 385. Raising either widens the whole screen.
...
- **Two small `draw(_:)` overrides are token-resolution, not new chrome:**
  ...
  chrome. The content pane's `panel` fill is `AudiouterSharedUI`'s
  `WarmPanelView`.
