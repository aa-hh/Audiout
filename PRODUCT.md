...
- **The app never blocks; unregistered builds ask, registered builds are quiet.** (Alec, 2026-08-23 — supersedes "never blocks, gates, or nags".) Settings has a licence-key field. A build without a valid key keeps every feature working but shows a "please buy" prompt; entering a purchased key removes it. The key is validated by an open-source signature check (no secrets in the app) and additionally gates the download portal and the Sparkle update feed server-side. A licence check-in records how many devices a licence appears on so Alec can *see* sharing — telemetry, not a gate.
