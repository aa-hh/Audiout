...
- The draw-diff machinery lives ONLY in `AudiouterCore/Sources/AudiouterSharedUI/BusRailOverlayView.swift`; its symbols (`EnergySignature`, `energizeSignature`, `connectPulseFires`, `reconcileEnergize(with:)`, `lastEnergy`, `lastMemberYs`, `settleBaseline`, `windowOcclusionStateDidChange`, `test_reconcileEnergize`, `RailPlan.memberYs`) are referenced by exactly two files: that file and `AudiouterCore/Tests/AudiouterCoreTests/RailConnectPulseTests.swift` (repo-wide `git grep`).
- TRAP — name collision: `PopoverController` has its OWN `private func reconcileEnergize()` (`AudiouterCore/Sources/AudiouterPopoverUI/PopoverController.swift:1662`, called at 2202). It prunes the item-9 energize pending beat and is UNRELATED to the overlay's `reconcileEnergize(with:)`. Do not touch it.
- `BusRailOverlayView` hosts: exactly TWO — `PopoverPanelViewController` (`AudiouterCore/Sources/AudiouterPopoverUI/PopoverPanelViewController.swift:227`, internal `let railOverlay`) and `GroupEditorViewController` (`AudiouterCore/Sources/AudiouterWindowUI/GroupEditorViewController.swift:59`). `MainOutDetailViewController` hosts NO overlay (grep confirmed). The editor only sets `mainOutRow`/`deviceRows`/`needsDisplay` (638–640) and calls `test_resolvePlan` (1028) — none of the removed symbols — and its `receiveRailPulse` is a no-op (`GroupEditorViewController.swift:1136`), so removing the draw-time reconcile just means the editor never pulses. That is the intended design.
- `RailNodeProviding` (declared `BusRailOverlayView.swift:1064-1071`) has exactly THREE conformers: `DeviceRowView` (`AudiouterCore/Sources/AudiouterSharedUI/DeviceRowView.swift:2922-2928`; has `public private(set) var device: Device` at 127), `MembershipRowView` (`AudiouterCore/Sources/AudiouterWindowUI/MembershipRowView.swift:483-487`; has `public private(set) var device: Device` at 62), and the test stub `StubRow` (`RailConnectPulseTests.swift:110-120`).
- `RailHookProviding` conformers: `MainOutRowView` (`AudiouterCore/Sources/AudiouterPopoverUI/MainOutRowView.swift:812-831`; `railHookAnchor` calls `layoutSubtreeIfNeeded()` at 820 and works headless — `PopoverControllerTests.swift:845` resolves a popover rail plan with no window), `GroupEditorViewController` (1103, `receiveRailPulse` no-op at 1136), and the test `StubHook` (`RailConnectPulseTests.swift:98-108`). No change needed to this protocol.
...
- `ConnectionState` is `off/connecting/connected/reconnecting/failed(ConnectionFailure)` (`AudiouterCore/Sources/AudiouterCore/ConnectionState.swift:22-28`); `Equatable`, so `$0.connectionState == .connected` compiles.
...
- Controller-test harness precedent: `PopoverDeviceVisibilityTests.makePopover` (`AudiouterCore/Tests/AudiouterCoreTests/PopoverDeviceVisibilityTests.swift:25-43`) — MockBackend + GroupController + `popover.test_isShownOverride = true`, then hand-built `popover.update(devices:)` pushes; device fixtures at 45-57.
...
File: `AudiouterCore/Sources/AudiouterSharedUI/BusRailOverlayView.swift`, protocol at 1064-1071.
...
- `AudiouterCore/Sources/AudiouterSharedUI/DeviceRowView.swift` extension at 2922-2928: add `public var railDeviceID: String? { device.id }`.
- `AudiouterCore/Sources/AudiouterWindowUI/MembershipRowView.swift` extension at 483-487: add `public var railDeviceID: String? { device.id }`.
...
File: `AudiouterCore/Sources/AudiouterSharedUI/BusRailOverlayView.swift`, place it in the "Connect pulse" MARK section (after `reduceMotion`, line 472).
...
File: `AudiouterCore/Sources/AudiouterSharedUI/BusRailOverlayView.swift`. Delete, with their doc comments:
...
File: `AudiouterCore/Sources/AudiouterPopoverUI/PopoverPanelViewController.swift`, immediately after `setRailRows` (ends line 396).
...
File: `AudiouterCore/Tests/AudiouterCoreTests/RailConnectPulseTests.swift`. Update the suite doc header to the new architecture. Per-test disposition:
...
- Track A (Steps 1–8): model **opus**, effort **medium** (multi-file surgery with a name-collision trap, but zero design decisions remain — everything is specified). Files: the 6 source/test files named above + `AudiouterPopoverUI/AGENTS.md` + 1 new test file.
