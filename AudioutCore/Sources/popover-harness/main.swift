...
/// of those instead — the type is internal to `AudioutPopoverUI`, and the
...
func run() -> Int32 {
    ...
    // The local Mac is never part of the BACKEND output set (AudioutCore/AGENTS.md
...
func runConnectionStatusChecks(_ checks: Checks) {
    ...
    // R12 (AudioutPopoverUI/AGENTS.md, W2-T3 — "keep intent, always"): a failure
...
func runFeedColumnChecks(_ checks: Checks) {
    ...
    checks.expectEqual(popover.test_deviceRow(for: "office")?.test_feedText, nil,
                       "a failed device's FEED column drops the composite for the glyph-only failure override")
    checks.expectEqual(popover.test_deviceRow(for: "office")?.test_feedTooltip, "Didn't respond",
                       "…and the headline moves to the tooltip (2026-09-04)")
}
