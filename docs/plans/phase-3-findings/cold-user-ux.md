# Cold-User UX Clarity Audit — Audiout (Phase 3 / A2)
...
touched (`AudioutCore/`, `AudioutCore/Sources/Audiout{SharedUI,
...
`AudioutOnboardingUI` has no `AGENTS.md` of its own; its contract lives in
`AudioutApp/AGENTS.md`'s "First-run onboarding" section, which was read in
...
A repo-wide search for `NWPathMonitor`/`Wi-Fi`/`isConnected` inside `AudioutCore/Sources/AudioutCore` and the popover/onboarding UI turns up no overall-network-state observer — only the per-device Bonjour/connection machinery. If the user's Wi-Fi drops entirely, the popover has no distinct code path to say so; it would present exactly the same "Looking for devices…" (Flow 7) or generic `.unknown` failure a normal empty network gives. Tagging for live confirmation since this is inferred from an absence, not an explicit negative-path test.
