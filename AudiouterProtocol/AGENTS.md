...
- `CompanionCommand` — the phone's 21 outbound commands, hand-Codable.
...
- `Snapshot` / `DeviceState` / `GroupState` / `AppRouteState` / `MainOutState` / `SettingsState` / `AlignmentProbeState` — full app state, `Equatable` for change-suppression on the server.
