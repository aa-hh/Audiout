...
repo AGENTS.md and `AudioutCore/AGENTS.md`, before touching code.*
...
New file `AudioutCore/Sources/AudioutCore/ConnectionState.swift`:
...
New file `AudioutCore/Sources/AudioutCore/ConnectionDiagnostics.swift`.
...
### 7.1 `ConnectionDiagnosisView` (T6 — new file in AudioutPopoverUI)
...
- Docs: SPEC.md §9 device-row table gains the status slot/sublabel/diagnosis
  ...
  `AudioutCore/AGENTS.md` updated if it describes the row anatomy or
...
Build/test from `AudioutCore/`: `swift build`, `swift test`,
