...
- **Chromecast (Google Cast) speakers are supported** (owner, 2026-09-05): a Cast speaker joins a synced group like any AirPlay or Bluetooth device, so the site and press kit list "AirPlay, Bluetooth and Chromecast". The one limit is the per-app one above: Cast takes the group mix, never a single app's audio.
...
1. **Anonymous feature telemetry — opt-in, off by default.** Asked once, never re-nagged (the last card of first-run Setup). Answers feature usage, feature discoverability, uninstall rate, and month-over-month users.
   ...
   **Also sent under the same opt-in (added 2026-09-06): the diagnostic log.** Every `Telemetry` line (permission checks, capture and AirPlay state changes, Cast session events, window-frame decisions) is forwarded to PostHog Logs as it is written to the local file, minus any field that names a device (`Analytics.deviceKeys`) and minus the two per-second Cast samplers (`Telemetry.localOnly`). Retained 14 days on PostHog's EU servers. Same random per-install ID, same off switch.
