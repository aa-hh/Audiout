...
- `BTAlignmentWizardSession` enters `.listening` only when its host's `requestListening` answers true; a rejected proposal never ends a run — only the wing rule, a stagnant belief, or the spent answer budget do. A failed first listen, or a MEASURED proposal's first rejection, listens again; the next failure or rejection falls to the by-ear questions. The mic runs at most `maxMicAttempts` times a run.
