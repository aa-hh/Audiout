...
It is a sibling package to `AudioutCore`, not a target inside it, because
`AudioutCore/Package.swift` shells out to `brew --prefix` and pulls in the
...
- **Zero dependencies, forever.** `Package.swift` here must never grow a
  ...
  this package exists apart from `AudioutCore`. If a future need seems to
  require one, it belongs in `AudioutCore` or a new package, not here.
