...
/// the speaker row's "New Scene from Selection…". Selection is reported
/// through `onSelect`.
///
...
public final class SidebarViewController: NSViewController {
    ...
    /// selection, and a speaker row's "New Scene from Selection…" context item
    ...
    public override func loadView() {
        ...
        // Bottom add bar with a labeled "New Scene…" affordance — the standard
    ...
    private func updateAddButtonTitle() {
        ...
        let title = count >= 2 ? "New Scene from \(count) Speakers…" : "New Scene…"
    ...
    /// "New Scene from Selection" gesture.
    ...
    /// The bottom-bar add button's current title — "New Scene…" plain, "New
...
/// no UI can advertise the shortcut. Upgrade path: a real "New Scene…" item in
...
extension SidebarViewController: NSMenuDelegate {
    ...
    public func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        case .groupsOverview:
            ...
            menu.addItem(contextMenuItem("New Scene…",
        ...
        case .device(let device):
            ...
            let title = ids.count == 1
                ? "New Scene from \u{201C}\(device.name)\u{201D}\u{2026}"
                : "New Scene from \(ids.count) Speakers\u{2026}"
