...
2. **License activation check-ins — identified by purchase.** Ties a license ID to a device count. This is not anonymous and must never be described as such. **Not opt-in** (owner's call, 2026-08-24): unlike stream 1, this is abuse detection — a license appearing on far more devices than one buyer plausibly owns — so it cannot be a toggle an abuser simply switches off. It runs unconditionally whenever a key and a check-in endpoint exist, the same way any paid app's license check does; users are told about it (this document, the source), not asked to consent to it.
