...
/// spirit of the Mac's own curated set (`AudioutSharedUI/DeviceIcon.swift`,
/// unreachable from iOS per `ios/AGENTS.md`'s `AudioutCore` import ban) with
/// a short iOS-side list: speakers, rooms, home symbols only, not the Mac's
/// full catalog.
enum GroupIconLibrary {
    /// Mirrors `Group.defaultIconSymbolName` (AudioutCore/GroupStore.swift)
