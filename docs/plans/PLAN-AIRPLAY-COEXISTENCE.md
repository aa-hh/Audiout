...
1. **No yield signal exists.** Measured live: when macOS loses the 319/320 bind race it does **not** change the Mac's default output device — it fails and leaves all audio state untouched, surfacing only a "Could not connect" dialog. So the `kAudioHardwarePropertyDefaultOutputDevice` listener (`SystemOutputVolume.swift:198`) — the one plausible detector, and the one G2b confirmed we already own — never fires. There is nothing to subscribe to. The kernel likewise offers no notification to a port holder that another process attempted a bind; a failed `bind()` is private to the loser.
