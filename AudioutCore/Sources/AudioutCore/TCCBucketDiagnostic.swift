...
/// Env-gated (`AUDIOUT_TCC_DIAG=1`), once-per-second ``Telemetry`` poll built
...
public enum TCCBucketDiagnostic {
    ...
    /// Starts the poll loop iff `AUDIOUT_TCC_DIAG=1`; a no-op otherwise, and
    ...
    public static func startIfEnabled() {
        guard ProcessInfo.processInfo.environment["AUDIOUT_TCC_DIAG"] == "1" else { return }
        lock.lock()
    ...
    private static let bogusServiceName = "kTCCServiceAudioutDiagBogusControl"

    private static let pollInterval: TimeInterval = 1.0
    private static let queue = DispatchQueue(label: "com.audiout.tccdiag")
