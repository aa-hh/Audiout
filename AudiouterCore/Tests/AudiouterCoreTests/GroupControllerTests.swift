...
@Suite struct GroupControllerTests {
    ...
    @Test func deselectingLocalItselfDoesNotSelfRestore() async throws {
        let (controller, _) = try await makeController()
        controller.ensureDefaultSelection()                       // {local}
    ...
    @Test func addAirPlay_whenSelectionEmpty_plainInsert() async throws {
        let (controller, _) = try await makeController()
        controller.ensureDefaultSelection()                       // S = {local-mac}
        ...
        let r = controller.setDeviceSelected("office", true)
        #expect(r.applied)
        #expect(!r.autoSwappedCurrentDevice, "no sole Mac member ⇒ no auto-swap")
