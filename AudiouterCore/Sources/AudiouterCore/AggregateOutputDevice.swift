...
// false`) aggregate device named "Audiouter" that appears in System Settings ›
...
/// aggregate's "off switch" — i.e. did the user turn Audiouter off, or did its
...
/// cleanup) of the public "Audiouter" aggregate device. Pure decision logic;
...
public struct AggregateOutputDevice: Sendable {
    ...
    /// lingering `com.audiouter.spike.aggregate` entry is a real, not
    ...
    static let priorSpikeUID = "com.audiouter.spike.aggregate"
    ...
    /// Audiouter off in favor of a real device, or its underlying device just
