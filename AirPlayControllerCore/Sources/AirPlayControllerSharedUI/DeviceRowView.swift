...
public final class DeviceRowView: NSView {
    ...
    public func apply(_ device: Device,
        ...
        enableSwitch.isEnabled = showsToggle && device.isAvailable && !blocked && !isUnsupported
        enableSwitch.toolTip = isUnsupported
        ...
        // an unsupported row never shows the accent tint even if `selected`
        // happens to be stale/true, since it's never actually routed.
        iconView.contentTintColor = (selected && !isUnsupported) ? .controlAccentColor : .secondaryLabelColor
        ...
        // available member (its level composes the Main Out master); never for
        // an unsupported device (it's never actually an output).
        slider.isEnabled = device.isAvailable && selected && !device.isMuted && !isUnsupported
        muteButton.isEnabled = device.isAvailable && selected && !isUnsupported
