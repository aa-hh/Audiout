...
4. **The AirPlay 1 row's tooltip consequence line is gone; the "Older AirPlay" tag stays.** `DeviceRowView`'s tooltip used to append "Older AirPlay — can't route single apps" whenever `isOlderAirPlayReceiver`; that line is deleted in `590dd033`. The tag itself, gated on the same `isOlderAirPlayReceiver`, is untouched — it still truthfully names the receiver kind.
