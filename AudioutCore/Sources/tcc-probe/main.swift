...
/// `TCCProbeRunner` (`AudioutCore/Sources/AudioutCore/TCCProbeRunner.swift`)
/// is what spawns this binary and parses its output.
///
/// Deliberately duplicates `SystemAudioCaptureTCC.preflight(service:)`'s
/// dlopen/dlsym resolution rather than linking `AudioutCore` and calling it:
...
private let bogusServiceName = "kTCCServiceAudioutProbeBogusControl"
