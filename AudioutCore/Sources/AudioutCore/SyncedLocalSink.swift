...
public final class SyncedLocalSink: @unchecked Sendable {
    ...
    private let graphQueue = DispatchQueue(label: "com.audiout.syncedlocalsink.graph")
    ...
    private let lifecycleQueue = DispatchQueue(label: "com.audiout.syncedlocalsink.lifecycle")
    #if canImport(AudioToolbox)
    ...
    /// Wired externally to `NSWorkspace.willSleepNotification`. `AudioutCore`
    /// must never import AppKit (package rule, `AudioutCore/AGENTS.md`), so —
