...
struct raop_master_session
{
  ...
  // [AirPlayEngine vendored change 2026-07-19] Per-app multi-stream routing for
  // the RAOP/AirPlay-1 sender — mirror of airplay.c's P2b surgery (see
  // docs/VENDORED-DIFFS.md Entry 3). Identity of the independent content stream
  // this master session carries. master_session_make keys the session cache on
  // (stream_id, quality, encrypt), so distinct stream_ids never share a master
  // session (one RTP timeline / encoder / input buffer per stream). Default 0 =
  // the single legacy stream, so the pre-change single-stream path is unchanged.
  uint32_t stream_id;
...
struct raop_session
{
  ...
  // [AirPlayEngine vendored change 2026-07-19] Per-app multi-stream routing
  // (mirror of airplay.c P2b). The content stream this device connection is
  // routed to; used to bind the session to the matching master session in
  // session_make. Default 0.
  uint32_t stream_id;
...
// [AirPlayEngine vendored change 2026-07-19] Per-app multi-stream routing
// (mirror of airplay.c's P2b master_session_make): added the leading `stream_id`
// parameter and keyed the session-reuse cache on (stream_id, quality, encrypt)
// instead of (quality, encrypt). This is what lets N independent-content streams
// coexist on RAOP: two devices routed to different streams get DIFFERENT master
// sessions even at the same quality/encrypt, so their RTP timelines / encoders /
// input buffers never collide. stream_id 0 = the legacy single stream.
static struct raop_master_session *
master_session_make(uint32_t stream_id, struct media_quality *quality, bool encrypt)
{
  ...
  for (rms = raop_master_sessions; rms; rms = rms->next)
    {
      if (rms->stream_id == stream_id && encrypt == rms->encrypt && quality_is_equal(quality, &rms->rtp_session->quality))
	return rms;
    }
  ...
  // [AirPlayEngine vendored change 2026-07-19] The engine defaults
  ...
  // alac_encode_no_xcode) and never the ffmpeg encoder. Upstream still built the
  // ffmpeg encode_ctx here unconditionally, which would make an AirPlay-1 session
  // fail to start without libavcodec's ALAC encoder even though the codec output
  // is never used. Guard the ffmpeg encoder setup on !raop_uncompressed_alac so
  // the RAOP path has no ffmpeg dependency (encode_ctx stays NULL and is only
  // ever passed to the guarded alac_encode_xcode branch; master_session_free's
  // transcode_encode_cleanup is NULL-safe). VENDORED-DIFFS.md Entry 3.
  if (!raop_uncompressed_alac)
    {
  ...
  rms->encrypt = encrypt;
  rms->quality = *quality;
  // [AirPlayEngine vendored change 2026-07-19] P2b (mirror): record the stream this
  // master session serves so the reuse cache above and raop_write's fan-out match it.
  rms->stream_id = stream_id;
...
// [AirPlayEngine vendored change 2026-07-19] P2b TEST/DIAGNOSTIC SEAM (mirror of
// airplay.c's airplay_test_master_session_* accessors). master_session_make() and
// the raop_master_sessions list are static, so the headless multi-stream unit test
// cannot otherwise observe master-session identity. These thin non-static accessors
// expose exactly that, calling the SAME master_session_make the production
// session_make uses (so the test exercises the real (stream_id, quality, encrypt)
// keying, not a reimplementation). None are reachable from any shipping code path.
// Prototypes in shims/engine_bridge.h.
void *
raop_test_master_session_make(uint32_t stream_id, struct media_quality *quality, bool encrypt)
{
  return master_session_make(stream_id, quality, encrypt);
}

uint32_t
raop_test_master_session_stream_id(const void *rms)
{
  return ((const struct raop_master_session *)rms)->stream_id;
}

// Sample count raop_write's fan-out has accumulated into this master session's
// (otherwise-static/opaque) input_buffer. Lets a headless test prove NO CROSS-TALK:
// feeding one stream_id never grows the other stream's input buffer.
uint32_t
raop_test_master_session_input_buffer_samples(const void *rms)
{
  return ((const struct raop_master_session *)rms)->input_buffer_samples;
}
...
{
  // In the test rig no raop_session references these, so master_session_cleanup
  // unlinks and frees each current head until the list is empty.
  while (raop_master_sessions)
    master_session_cleanup(raop_master_sessions);
}
...
session_make(struct output_device *rd, int callback_id, bool only_probe)
{
  ...
  // [AirPlayEngine vendored change 2026-07-19] P2b (mirror of airplay.c): carry
  // the device's routed stream onto the session so it binds to the matching
  // master session below.
  rs->stream_id = rd->stream_id;
  ...
  // [AirPlayEngine vendored change 2026-07-19] P2b (mirror): bind to the master
  // session for this device's routed stream (was keyed on quality+encrypt only).
  rs->master_session = master_session_make(rs->stream_id, &rd->quality, rs->encrypt);
  if (!rs->master_session)
    {
...
packets_send(struct raop_master_session *rms)
{
  ...
  for (rs = raop_sessions; rs; rs = rs->next)
    {
      // [AirPlayEngine vendored change 2026-07-19] P2b audit (no code change):
      // this send site matches on master_session POINTER identity, which is
      // strictly stronger than a stream_id compare — a session bound to `rms`
      // necessarily carries `rms->stream_id` (session_make binds via
      // master_session_make on the same stream_id). So it is already stream-safe;
      // the stream_id discriminator only had to be added where matching was by
      // stream_id (the raop_write fan-out above).
      if (rs->master_session != rms)
	continue;
...
raop_write(struct output_buffer *obuf)
{
  ...
  for (rms = raop_master_sessions; rms; rms = rms->next)
	...
	{
	  // [AirPlayEngine vendored change 2026-07-19] P2b (mirror of airplay.c):
	  // a PCM blob reaches a master session only when BOTH its stream_id and
	  // its quality match. stream_id selects WHICH content stream (per-app
	  // routing); quality still selects the encoder format. Without the
	  // stream_id guard, every stream's audio would fan out to every master
	  // session of the same quality = silent cross-talk (the pitch-up/crackle
	  // symptom). All RAOP app streams share one quality (44100/16/2), so the
	  // quality check alone cannot disambiguate them — stream_id is the only
	  // discriminator. Default 0 == 0 keeps the single-stream path exact.
	  if (obuf->data[i].stream_id != rms->stream_id)
	    continue;
...
// [AirPlayEngine vendored change 2026-07-19] P2b TEST SEAM: drive the REAL
// raop_write fan-out with a single-stream output_buffer so a headless test can
// prove the stream_id gate routes a PCM blob only into the matching master
// session. Builds a one-entry output_buffer (the shim struct) with the given
// stream_id/quality and calls raop_write exactly as the player would. Defined
// after raop_write so it sees the real (static) definition. NOT shipping API.
void
...
{
  struct output_buffer obuf = { 0 };

  obuf.pts.tv_sec = pts_sec;
  obuf.pts.tv_nsec = 0;
  obuf.data[0].stream_id = stream_id;
  obuf.data[0].quality = *quality;
  obuf.data[0].buffer = (void *)pcm;
  obuf.data[0].bufsize = bufsize;
  obuf.data[0].samples = samples;
  // data[1].buffer stays NULL — the fan-out loop's terminator.
