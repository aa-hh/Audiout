...
- **D1 The shared ramp is used in BOTH appearances.** `AudioutField.ramps["Chill out"]` carries one `lo`/`mid`/`peak` triple set and nothing appearance-specific; `paperLift` (already applied at `EmitterFieldView.swift:678`, `light *= mix(1.0, paperLift, u.lightMode)`) is the shared field's own answer to a light ground. So light mode stops inverting the ramp and differs only by `bg` (which stays `Tokens.Color.canvas`, resolving per appearance) and that lift. This is the point of the change: the Mac authors no ramp at all.
- **D2 The field stops following the accent dial.** Today `makeUniforms` reads `Tokens.Color.gold` (`:508`), so Subtle desaturates the field. The shared ramp is fixed hexes, so after this PR only `bg` moves with appearance and nothing moves with the dial. Accepted — the ramp is brand data, not app state. Consequence: `EmitterFieldView.swift:223`'s `accentStyleDidChangeNotification` observer no longer has a colour to re-stamp; leave it in place — it is on the same handler as the appearance re-stamp and removing it is not this PR's business.
- **D3 Missing ramp key = `fatalError`,** matching the shared package's own idiom (`Field.swift:60`, `:66` both `fatalError("… the package is broken.")`). A `"Chill out"` key that has gone missing is a broken dependency pin, not a runtime condition.
