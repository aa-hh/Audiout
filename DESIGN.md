...
**Recommended, pending Alec's decision: keep the flat ground and move the ladder from FILL to EDGE WEIGHT.** One mechanism, the hairline, at two weights — a stronger `#C4C0B4` (1.76:1 vs panel) for a container's own edge, today's `#D0CDC3` (1.54:1) for dividers inside it. `well` and the grouped-inset geometry are unchanged. Nothing in any UI target is drawn *on* a hairline — it is only ever a stroke or a divider fill — so this costs **zero** instrument contrast and **zero** text contrast, which is what disqualifies the alternatives: deepening `well` drops `gold`, `ember`, `caution` and `faderThumb` under 3:1 *and* kills the container border it was meant to help, and inset geometry alone measures 1.000:1 because with flat fills and no edge there is no boundary pixel at all.

**Until Alec decides, change neither ground.** Both are load-bearing and both were reasoned.
