...
AppKit row views shared by the popover (`AirPlayControllerPopoverUI`) and the mixer window (`AirPlayControllerWindowUI`) so both hosts render the same device row from one implementation. Views here are pure UI: controls route out through a per-view `Delegate` (or closure), never touching a backend, store, or `GroupController` directly. For the routing model these rows render, see [../../AGENTS.md](../../AGENTS.md).
