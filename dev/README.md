...
|---|---|---|---|
| **In-app mock backend** | `../AudiouterCore` | All UI + control logic: discovery, multiple devices, groups/output-sets, per-device volume/mute/solo, level meters, drop/reconnect | The wire/audio path (by design) |
...
The app links `AudiouterCore` and talks only to the `OutputBackend`
