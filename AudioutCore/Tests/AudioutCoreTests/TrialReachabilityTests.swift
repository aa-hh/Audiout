...
/// trial is registered or expired watches forever for something that can never
/// happen. And a monitor that stops on a Mac with no trial yet leaves the
/// welcome gate's brand-new trial with no retry until the next launch — the
/// exact case the network was most likely down for.
@Suite struct TrialReachabilityTests {
    ...
    func unregisteredOnSatisfiedPathRegisters() {
        ...
        #expect(TrialReachability.step(for: state, satisfied: true) == .register)
    }
    ...
    func unregisteredOnUnsatisfiedPathWaits() {
        let state = TrialState.active(daysLeft: 14, expiresAt: Self.expires, registered: false)
        #expect(TrialReachability.step(for: state, satisfied: false) == .wait)
    }
    ...
    func registeredStops() {
        ...
        #expect(TrialReachability.step(for: state, satisfied: true) == .stop)
    }
    ...
    @Test("A Mac with no trial keeps watching")
    func noTrialWaits() {
        #expect(TrialReachability.step(for: .none, satisfied: true) == .wait)
    }
}
