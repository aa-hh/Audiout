...
final class WorkgroupLifecycleTests: XCTestCase {
    ...
    func testUpdateWorkgroupIsNotSchedulableBeforeStart() {
        ...
        XCTAssertFalse(thread.updateWorkgroup(deviceID: 42))
        XCTAssertFalse(thread.updateWorkgroup(deviceID: nil))
    }

    func testJoinAgainstACancelledDeviceIsSkippedNotFatal() throws {
        ...
        let fence = expectation(description: "join attempt completed on the engine thread")
        _ = thread.enqueue({ fence.fulfill() }, tracked: false)
        wait(for: [fence], timeout: 5.0)
    }

    func testRepeatedUpdatesAndLeavesAreSafeAndOrdered() throws {
        ...
        for id in [noSuchDevice, 1, noSuchDevice, 2] {
            XCTAssertTrue(thread.updateWorkgroup(deviceID: id))
        }
        XCTAssertTrue(thread.updateWorkgroup(deviceID: nil))
        XCTAssertTrue(thread.updateWorkgroup(deviceID: nil), "leave is idempotent")

        let fence = expectation(description: "churn drained on the engine thread")
        _ = thread.enqueue({ fence.fulfill() }, tracked: false)
        wait(for: [fence], timeout: 5.0)
    }

    func testThreadStopsCleanlyAfterAJoinAttempt() throws {
        ...
        XCTAssertTrue(thread.updateWorkgroup(deviceID: noSuchDevice))
        let fence = expectation(description: "join attempt completed")
        _ = thread.enqueue({ fence.fulfill() }, tracked: false)
        wait(for: [fence], timeout: 5.0)
    ...
    func testEngineWithNoThreadReportsWorkgroupCallsAsUnscheduled() async {
        ...
        XCTAssertFalse(engine.joinIOWorkgroup(deviceID: 1))
        XCTAssertFalse(engine.leaveIOWorkgroup())
    }
}
