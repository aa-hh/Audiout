# AGENTS.md history: AudioutCore/Sources/AudioutSettingsUI
...
- **The sizing traps — probe-confirmed AppKit facts. Do not weaken any of
  ...
  **Probed 2026-08-12 (roadmap 050, the Advanced disclosure): a pane's own
  ...
  uses the app's one collapse idiom, the `CardView` clip (AudioutPopoverUI):
  ...
  `FoldAnimator.shared` (public in `AudioutSharedUI`), the app's one fold
...
- The pane host puts an explicit opaque, appearance-adaptive background behind
  ...
  canvas unification that background is `WarmPanelView` (`AudioutSharedUI`)
