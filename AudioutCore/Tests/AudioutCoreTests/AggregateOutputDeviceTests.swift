...
/// Hermetic tests for the Wave 3 PUBLIC "Audiout" aggregate's pure, injectable
    ...
    @Test func sweepOrphansDestroysBothKnownUIDsThenIsANoOpOnRerun() {
        let control = FakeAggregateControl(
            resolvable: [
                ...
                "com.audiout.spike.aggregate": 777,
            ],
            enumerated: [AggregateOutputDevice.productUID, "com.audiout.spike.aggregate"])
...
@Suite struct AggregateOutputDeviceWiringTests {
    ...
    /// flips the warning on; the user clicking the popover's "Use Audiout"
    ...
    @Test func routingBlockedTracksUserDeselectThenAppReselect() async {
        ...
        // The user clicks "Use Audiout" — re-takes the default and
    ...
    /// CASE 6 — the popover's "Use Audiout" button resumes: re-takes the
    ...
    /// CASE 7 — the D1 fix: re-picking Audiout directly in Sound settings (no
    ...
    @Test func soundSettingsRepickResumesWithoutTheButton() async {
        ...
        #expect(events.contains { self.isRoutingBlocked($0, false) }, "D1: re-picking Audiout directly must resume too")
...
/// quitting a side-by-side test build swept the "Audiout" output device out
...
@Suite struct AggregateOutputDeviceIdentityTests {
    ...
    @Test func theShippingBundleKeepsItsPersistedUIDAndName() {
        #expect(AggregateOutputDevice.shippingUID == "com.audiout.Audiout.aggregate",
                "coreaudiod keys its persisted entry by this exact string — changing it orphans every existing entry")
        #expect(AggregateOutputDevice.shippingBundleID == "com.audiout.Audiout")
    }
    ...
    @Test func aSideBuildOwnsItsOwnAggregateUID() {
        let sideUID = AggregateOutputDevice.productUID(forBundleID: "com.audiout.Audiout.syncv2")
        #expect(sideUID == "com.audiout.Audiout.syncv2.aggregate")
    ...
    @Test func anUnbundledRunKeepsTheShippingUID() {
        ...
        #expect(AggregateOutputDevice.productUID(forBundleID: "com.audiout.Audiout")
                == AggregateOutputDevice.shippingUID)
    }
}
