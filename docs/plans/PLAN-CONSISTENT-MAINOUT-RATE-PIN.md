...
**What Audiout is.** A native AppKit macOS menu-bar app that captures the Mac's whole-system audio and streams it to one or more AirPlay 2 speakers. The shipping audio backend is `NativeBackend` (`AudioutCore/Sources/AudioutCore/NativeBackend.swift`), which drives a vendored AirPlay 2 sender (`AirPlayEngine`). "Main Out" is the product name for the whole-system capture → AirPlay pipeline (as opposed to per-app redirects).

**How whole-system capture works (the pipeline this plan touches).** `NativeCaptureCoordinator` (`AudioutCore/Sources/AudioutCore/NativeCaptureCoordinator.swift`, 3377 lines) owns an in-process Core Audio capture pipeline:
...
There is **one process-wide watcher**, `DefaultOutputDeviceMonitor` (`AudioutCore/Sources/AudioutCore/DefaultOutputDeviceMonitor.swift`), that owns the single HAL listener pair for `kAudioHardwarePropertyDefaultOutputDevice` (device identity) and `kAudioDevicePropertyNominalSampleRate` (device rate), with a 1.2 s **settle window** (leading + trailing, F-SETTLE) that absorbs the BT negotiation burst. It fans out to subscribers.
...
Two parts, both behind one env flag `AUDIOUT_PIN_CAPTURE_RATE` (model the flag on the existing `AUDIOUT_PAUSE_ON_CALL` pattern at `NativeCaptureCoordinator.swift:2441`: `static let pinCaptureRateEnabled = ProcessInfo.processInfo.environment["AUDIOUT_PIN_CAPTURE_RATE"] == "1"`).
...
**Spike scope (minimal, all behind `AUDIOUT_PIN_CAPTURE_RATE=1`, flag OFF = today byte-for-byte):**
...
- Build the real `.app` (`scripts/make-app.sh` — required; a bare `swift run` loses the TCC grant). Fully quit any running Audiout first (single-instance rule, §9).
...
- Pinned: relaunch WITH `AUDIOUT_PIN_CAPTURE_RATE=1` (via the `launchctl setenv`-then-`open` path the team already uses for env-gated builds), repeat the BT connect.
...
All tasks live in `AudioutCore/Sources/AudioutCore/NativeCaptureCoordinator.swift` and its test file `AudioutCore/Tests/AudioutCoreTests/NativeCaptureCoordinatorTests.swift`, so they **serialize** (same-file collision) — do them sequentially, not in parallel. This is a small, focused change; one agent end-to-end is the right shape.
...
| **T4** | **Docs**: update the two AGENTS.md that describe this path — `AudioutCore/AGENTS.md` (the `TapRebuildDecision` rules region and the Map entry near line 527) and `AudioutCore/Sources/AudioutCore/AGENTS.md` (the capture narrative) — to record the pin invariant, the flag, "whole-system only", and the fallback. Docs land WITH the code in the same branch (docs-first house rule). Add a short note to memory per project convention (not a repo file). | AGENTS.md files. | T2 | Sonnet, **low** |
...
- **Only ONE live native test instance at a time.** The PTP helper binds UDP ports 319/320 exclusively — fully **quit any running Audiout** (including the `/Applications` copy, which is the user's live build — do not touch or overwrite it) before a live test. Build a side-by-side copy via `scripts/make-app.sh` with `APP_NAME`/`BUNDLE_ID` overrides if needed. Before native live tests, sweep stale PTP daemons: `scripts/purge-stale-ptp-helpers.sh` (the `--apply` form needs sudo, so it is owner-run, never mid-session).
...
- **The pre-commit hook runs the full suite** (`scripts/run-tests.sh`, Guard 4) whenever staged files touch AudioutCore Swift sources/tests, and blocks on failure. Budget for it. Use `swift test --filter NativeCaptureCoordinatorTests` for the inner loop; never a bare `swift test` for a full run (use `scripts/run-tests.sh`).
...
- **Do not reintroduce a bare `44100`** — use `PCMFormat.airplay.sampleRate`. Do not add a second env-flag style; mirror `AUDIOUT_PAUSE_ON_CALL` exactly. Mark the deliberate whole-system-only ceiling with a `razor:` comment naming the per-app upgrade path.
