...
| `LocalNetworkPriming` / `LocalNetworkPrimer` | Seam + impl: fires the Local Network prompt and proves BOTH answers without a status API (TN3179). Grant = SELF-DISCOVERY — publishes `_audiouter-preflight._tcp` via `NWListener` and browses for its own unique instance, which works on a network with no speaker on it; denial = a browser reaching `.waiting(.dns(kDNSServiceErr_PolicyDenied))`; neither = `.undecided`. The `_airplay._tcp` browse still runs alongside for the speaker COUNT. The preflight type MUST stay listed in `NSBonjourServices` (`scripts/make-app.sh`) or the self-browse is silently blocked. |
