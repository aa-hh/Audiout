...
merged. `main` is merge-only — this branch reaches it via the owner's explicit
go-ahead only.
...
Devices card, run against the owner's own live feedback, not just automated
review. In order:
...
4. Four design decisions from the owner, live-checked on real builds: "Feed" →
   "Source", "Sync" → "Offset", This Mac pinned under the card header (no
   more one-row subsection), dark-mode dot socket `#34302A` → `#4A443B`.
5. Two more rounds of the owner's live feedback on the resulting build: the
...
7. Built `scripts/livetest.sh`, a machine-wide slot so parallel agents don't
   clobber the owner's in-progress hardware test of the shared dev bundle id —
   see §5.

## 3. What's still owed before merge

- **The owner's live check of the CURRENT build is not done.** The last thing
...
**The owner was mid-sequence to prove that persistence claim when this session
paused:** they ran `sudo sfltool resetbtm` (wipes the whole BTM database,
system-wide) and were restarting the Mac. The plan, unfinished as of this
write-up:

1. ~~`sudo sfltool resetbtm` + restart~~ — the owner confirmed they ran the command;
...
4. The owner's live check of the popover (§3).
5. Merge go-ahead.
...
the owner and the hold expired on its own. A peer session (branch
`claude/cast-delay-sync-proposal-7737fe`, roadmap 006 Cast sync) was told to
wait rather than take a fresh id, since the owner's permissions sequence took
...
to warn and ask, not silently take over, in case the owner is mid-test through
that other worktree. If genuinely stuck, `bash scripts/livetest.sh done
--force` clears it (say so to the owner first if you're not the one who just
...
# 4. Launch, let the owner approve permissions once
...
# 6. The owner live-checks the popover (Source/Offset alignment, hover grow-to-
#    post-click-size, header nesting, dark-mode dot). Release when done:
bash scripts/livetest.sh done

# 7. Merge only on the owner's explicit go-ahead.
```

## 7. Loose end for the owner, not blocking
