...
public final class PopoverController: NSObject {
    ...
    /// transient. State 4 used to have none either (the old copy claimed the
    /// app would "try again in a moment" on its own, which it never did) — the
    /// device is genuinely `.failed` by the time this state shows, so "Try
    /// Again" gives the user the same single-device re-kick a `.failed` row's
    /// own diagnosis panel offers.
