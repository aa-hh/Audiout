...
## EXECUTED 2026-07-24 (memory-leak-investigation branch) — status + adversarial findings

Implemented as Wave 3 of the memory-leak effort. T1 (new `AudioProcessResolver`),
T2 (per-app capture targets the full process-object set), T3 (whole-system tap
exclusion uses the full set), T4 (wiring), T5 (`.currentDevice` follows real
default with an AirPlay-refusal guard) — all landed, built, and full-suite green
(939 tests). Two of the five pieces (T1, T5) then went through a dedicated
adversarial correctness review; the findings below are what the LIVE test must
watch for. **None blocks the commit** (Firefox/Chrome — the scoped targets —
work; the memory-leak fixes are independent and already committed).

### T7 diagnostic — run this FIRST (silent, no audio)
`swift run process-audio-dump` (from `AudiouterCore/`) while the browser under
test is actively playing audio. It dumps every live Core Audio process object
with pid / command / parent / whether it independently resolves as an
`NSRunningApplication`. Confirms on YOUR machine whether the browser's audio
child is a ppid-descendant of its main process (the assumption the fix rests on).

### KNOWN FINDING — Safari/WebKit is NOT covered (scope limitation, not a defect)
The resolver walks UNIX ppid ancestry. Firefox/Chrome fork/exec their audio
helpers, so this works (Firefox verified live: helpers report the main firefox
pid as parent). **Safari/WebKit does NOT** — its audio processes are XPC services
reparented to launchd (`ppid == 1`), so a walk from Safari's main pid never reaches
them. Per-app redirect of Safari therefore captures nothing. This matches the
plan's scope (Firefox/Chrome only) and is net-neutral for Safari (it never worked).
No unit test can catch it. Forward path if wanted later, both in-Core (no AppKit):
match by `kAudioProcessPropertyBundleID`, or resolve the responsible-pid via the
private API TCC uses (dlsym; precedent = `TCCAccessPreflight`). **Live test:
target Firefox and Chrome, not Safari.**

### KNOWN FINDING — `.currentDevice` (T5) live behaviours to watch
- **Extra ~300 ms reconnect tick after every output-device switch.** Switching the
  Mac's output (unplug Bluetooth / change Sound output) while a "play on this Mac"
  app plays should recover on the new device, then produce ONE brief extra
  glitch ~300 ms later (a self-terminating second config-change recovery). Expected,
  self-limiting. If you also test whether `setDeviceID` posts a config change at
  all on your hardware, that resolves whether this finding is even real.
- **Mac with NO built-in speaker + AirPlay default:** local playback lands on the
  AirPlay device (feedback). PRE-EXISTING (old hard-pin code did the same); needs a
  Studio/Pro/headless Mac to hit. Correct fix has a product component ("play on this
  Mac" on a speaker-less Mac). Not fixed — decision + live confirm first.
- Lower-severity, live-verify: HAL process-object-list visibility under this app's
  entitlement posture (T1-F5); a Virtual device bridging to AirPlay slips the
  transport-type guard (T5-F3). Neither expected on a normal Sonos/HomePod test.

### Hardened from the reviews (landed)
- Resolver now guards `rootPID <= 1` at its public entry points (T1-F4): a malformed
  upstream pid (0/1/-1) resolves to empty instead of mass-matching launchd's whole
  tree. Unit-tested.
