...
@Suite final class GroupsWindowTextColorLockTests: IsolatedSuite {
    ...
    ///
    /// - ``Tokens/Color/label2`` was the first: the stock `secondaryLabelColor`
    ///   sits under the text floor in light, so this token is authored in both
    ///   appearances instead.
    /// - ``Tokens/Color/label3`` is the second. `tertiaryLabelColor` is black at
    ///   25.88% alpha, so it COMPOSITES against whatever it sits on: 1.87:1 on
    ///   `panel`, 1.86:1 on `raised`, 1.84:1 on `well`. Sweeping every possible
    ///   light ground from mid-grey to pure white, the best ratio the alias can
    ///   reach is 1.88:1 — the 4.5:1 floor is unreachable at that alpha on any
    ///   surface that could ever be painted under it. `label3` is authored,
    ///   ships all four variants, and measures 5.60:1 on the light flat ground /
    ///   4.86:1 on light `well` / 5.25:1 on dark `raised`.
    ///
    ...
    private var stockSemantics: [(name: String, color: NSColor)] {
        [
            ...
            ("Tokens.Color.label2 (measured-floor carve-out)", Tokens.Color.label2),
            ("Tokens.Color.label3 (measured-floor carve-out)", Tokens.Color.label3),
        ]
    }
