...
final class DemoPaneView: NSView {
    ...
    private static func makeMock(step: SetupStep?, mode: DemoMode) -> NSView {
        ...
        // Remote Control's retry raises the PROMPT again, not the pane — so its
        // Settings mode is the two-stage handoff, prompt first.
        case .settings: return step == .remoteControl
            ? DemoSettingsHandoffMockView(step: step)
    ...
    /// Non-nil only for the two-stage retry, whose settled frame is the ASK.
