...
| Status slot (2026-07-16; spinner+icon revised 2026-07-17) | `ArcSpinnerView` (custom, see below) / `NSImageView` (SF Symbol `circle.fill`) / borderless imageOnly `NSButton` (SF Symbol `exclamationmark.triangle`, outlined, `.medium`) | Fixed-width column between the `%` readout and the ENABLED switch. Spinner while connecting/reconnecting; persistent green dot once connected; orange **outlined** warning triangle (a real button, not just an image) on failure — tapping it toggles the inline diagnosis panel. Empty while `.off`. **The spinner is the ONE sanctioned exception to this section's documented-controls standing rule** (Alec, 2026-07-17): a custom `CAShapeLayer` 270° arc rotated by `CABasicAnimation`, chosen over both native candidates (`NSProgressIndicator`'s gear; SF-Symbol `rays` + macOS-14 `.variableColor` effect) after seeing all three rendered. The true ring symbol `progress.indicator.spinner` + `.rotate` effect is macOS 15+ — revisit when the deployment floor rises. Rationale + evaluation recorded in `ArcSpinnerView.swift`. |
