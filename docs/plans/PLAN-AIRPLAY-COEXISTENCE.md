...
Status: **WAVES 1–2 BUILT AND COMMITTED on `claude/airplay-audio-dropdown-1d717f`; NOT MERGED. Wave 3 planned, not started.** T1/T1b `40f49b6` · T3 `e05f5a1` · T2 `4c7da45` · T2b `7295940` · T4 `1298a70` · T5 `c437b26` · T6 `bee8ef1` · T7 `281cd66` · docs `25a22f4`. Combined tree green (AirPlayEngine 179, AudioutCore 1529). **The owner's live test is owed before any merge** — see "Wave 4 — combined live checklist"; nothing merges without their explicit go-ahead (standing rule, doubly so here: this rewrites the lifecycle of the one root component).
Author in a dedicated worktree/branch off `main`; `main` is merge-only. Merge only after the owner live-tests and explicitly says go (standing rule — doubly so here: this rewrites the lifecycle of the one root component). *(No driver-class component after all — decision 3's mechanism became the aggregate device; see its SUPERSEDED MECHANISM note.)*

Problem being fixed: Audiout's root PTP helper binds UDP 319/320 at boot (`RunAtLoad` + `KeepAlive`) and holds them forever, even when the app isn't running. macOS's own AirPlay 2 sender needs the same ports, so **installing Audiout permanently breaks the system AirPlay dropdown** (confirmed live 2026-07-26: `ptp-helper` PID 618 held `*.319`/`*.320` with Audiout not even running). Users will use both paths; they must coexist.

---

## Decisions locked (2026-07-26)

1. **Core principle = "last intent wins," automatically.** The PTP ports are a turn-taking resource — the two senders can never hold them simultaneously. Whoever the user touched last (system dropdown vs. Audiout device row) gets the ports, with no error states and no manual cleanup in either direction.
2. **Takeover is automatic, no confirmation dialog.** Clicking a device in Audiout while macOS AirPlay is playing ends the system session and starts ours, with a brief "Taking over from AirPlay…" transient state. The click *is* the consent.
3. **Sound-settings presence, spike-gated, shipped in this same combined effort.** Audiout appears as an output device in System Settings › Sound while active: users can see that's where audio goes and unselect it there (that's the off switch); the app re-selects it automatically on next use — the user never has to re-select manually. One combined plan (the owner chose combined over port-fix-first).
...
**Known asymmetry — INVESTIGATED AND CLOSED 2026-07-26, do not re-open without new evidence.** We can yield the ports only when idle. If the user clicks the *system* dropdown while an Audiout stream is active, macOS loses the bind race and its connect fails. The owner asked whether this could be made symmetric ("if something tries to take it from us, we give it up"). Three independent findings say no:

1. **No yield signal exists — measured TWICE, in both configurations.** (a) With built-in speakers as the default output and the ports held, macOS's failed AirPlay connect did **not** change the default output device; it left all audio state untouched behind a "Could not connect" dialog. (b) The owner then raised the sharper case: with **our own aggregate as the selected output**, would macOS deselect us *first* and then connect — giving us the signal in time? Tested directly (aggregate created and set default via the G2b `aggtool`, ports held by the probe, the owner picked the Sonos): `aggtool watch`, an event-driven listener on `kAudioHardwarePropertyDefaultOutputDevice`, recorded **zero** change events. macOS goes straight to the bind, fails, and touches nothing. So the listener (`SystemOutputVolume.swift:198`) never fires in either configuration — there is nothing to subscribe to. The kernel likewise offers no notification to a port holder that another process attempted a bind; a failed `bind()` is private to the loser.
...
**SUPERSEDED — 2026-07-27:** The owner approved building it best-effort as Wave 3 T9+ (seamless handoff). Shipped with an explicit degrade-to-release-on-deselect contract: blocking signal captured via `AirPlayHandoffWatcher` (unified-log watcher for `subsystem com.apple.airplay category APSNetworkClockPTP error kIOReturnExclusiveAccess`), handoff initiated, ports released via `PTPHelperReleasing` (~1s), so the user's own second click succeeds (nothing auto-retries — macOS abandoned the first attempt before we could react). Accepted ceiling: fail-once-retry-works — macOS gives up ~0.9s after the click and does NOT retry (measured 6/6 over 24h in unified log, zero spurious hits).

**Accepted mitigation (unchanged, and now the considered answer rather than a fallback):** the helper holds the ports *only while actually streaming* (Wave 1's idle-exit — verified live on real ports: bound 09:10:51, self-released 09:11:36, exactly the 30 s grace + 15 s idle window), so the system dropdown works in every case except during active Audiout playback. Wave 3 then makes that remaining case *explicable* rather than mysterious: Sound settings shows "Audiout" as the selected output, with deselecting it as the off switch.

**The half of the owner's idea that DOES work — build it in Wave 3 (T9).** Switching to a **non-AirPlay** output (speakers, headphones, USB) needs no PTP, so that switch genuinely succeeds, the default output really changes, and our listener *does* fire. Use it: on losing the default-output selection, end the session and release the ports **immediately**, rather than waiting out the 15 s idle window. Same off-switch semantics T9 already planned, but prompt instead of delayed. Only the direct system-dropdown-to-AirPlay case is unreachable.

**RESOLVED RISK — volume on the aggregate. Decision (owner's call, 2026-07-26): keep the aggregate, add real media-key interception.**
...
## Wave 4 — combined live checklist (the owner, before merge)

Fresh boot →
1. System dropdown → Sonos: works with Audiout installed but unused (Direction A).
2. While system-AirPlay plays: click a device in Audiout → auto-takeover, "Taking over…" shown, audio moves, Sound settings now shows Audiout selected.
3. Unselect Audiout in Sound settings → our session ends; ~1-2s later the system dropdown works again.
4. Reconnect in Audiout → virtual device re-selected automatically, no manual re-pick.
...
## Live checklist — seamless handoff (the owner, 2026-07-27)

- FIRST: purge stale daemons — `scripts/purge-stale-ptp-helpers.sh --apply` (standing rule before native live tests).
- Start streaming to a real speaker from Audiout; Sound settings shows "Audiout" selected.
- (A) Pick the built-in speakers in Sound settings. Expect: session ends ~1s; popover warning with "Use Audiout" appears; within ~2s `sudo lsof -nP -iUDP:319 -iUDP:320` shows nothing held. Then pick an AirPlay speaker in the Sound dropdown — expect it to connect on the FIRST try.
- (B) Re-stream from Audiout. Go STRAIGHT to the Sound dropdown and pick an AirPlay speaker. Expect: first attempt fails (macOS's own dialog), second attempt succeeds. Cross-check: `/usr/bin/log show --last 5m --predicate 'subsystem == "com.apple.airplay" AND category == "APSNetworkClockPTP"' --style compact` shows exactly one kIOReturnExclusiveAccess line for the failed attempt, none for the successful one.
- (C) Click "Use Audiout" in the popover. Expect: Audiout becomes the Mac's output again AND the previously selected speakers reconnect on their own AND any per-app redirect resumes (Option B) — no manual re-picks.
- (C2) Variant: instead of the button, re-pick "Audiout" directly in Sound settings — expect the same full resume (this was review finding D1).
- Regressions to sweep: rapid speaker toggle; switch speaker A→B; a per-app redirect live through an (A)/(B) handoff and resume; sleep/wake while handed over (Audiout must stay quiet, not re-grab).
...
**Built solution — PTPHelperReconciler (AudioutCore/Sources/AudioutCore/PTPHelperService.swift):** A launch-time, native-backend-gated, one-shot XPC liveness probe that fires immediately at app startup. It distinguishes the zombie case (XPC_ERROR_CONNECTION_INVALID event — launchd has no MachServices entry) from *legitimate contention* (helper fine but busy — any other XPC error or valid-but-not-ready connection reads `.indeterminate` and is NEVER healed). On a zombie verdict, it auto-repairs via `unregister()` → `register()` and logs Telemetry .airplay event `ptp_zombie_heal` (before/after status). If the repair leaves status `.requiresApproval`, AppDelegate immediately presents the setup screen (`presentSetup(reason: .permissionLost([.ptpHelper]))`) so the user finishes in Login Items without further prompting. Status: **built, unit-tested with injected fakes; live re-test owed** (SMAppService.enabled only reachable under Developer ID signing — ad-hoc builds cannot verify; needs a signed-build live pass).

**LIVE ROOT CAUSE — the every-launch heal loop (2026-08-06, signed build, isolated `com.audiout.zombietest` identity):** the first live pass proved the detection AND exposed a self-inflicted loop: telemetry showed a "successful" heal on EVERY launch (4/4, `before: enabled, after: enabled`) though only 2 zombies were created by hand. Forensics (unified log + two controlled experiments) ruled out a probe false-positive — launching with a live job heals nothing, and the job survives both helper idle-exit and app quit. The real mechanism: `SMAppService.unregister()` returning only means BTM *accepted* the teardown; the record/launchd removal keeps draining up to ~30s afterward. A `register()` accepted mid-drain ("Register error: 1", success 0.5s later) creates a **doomed registration** — the still-queued removal later executes against it (smd: "remove succeeded (EINPROGRESS)"), SIGTERMs the RUNNING helper mid-session (audible risk: kills the live PTP clock), and deletes the job, so the next launch finds a fresh REAL zombie and heals again, forever. A register landing AFTER the drain sticks and KEEPS approval (proven by the one surviving heal + a 25-min soak). **Fix (committed with this entry):** the heal now gates `register()` on the teardown being observably drained — it polls until status reads `.notRegistered` (same delay/attempt knobs as the register retry) and refuses to register into an un-drained teardown, reporting `.healFailed` (manual-recovery screen) instead of manufacturing the next launch's zombie; telemetry gains `drain_polls`.
...
- **Takeover under real contention** — macOS playing to the Sonos via its own AirPlay, then clicked the speaker in Audiout Coexist: takeover succeeded, audio moved over. Confirmed.

**Not yet covered live:**
- **T-ZOMBIE auto-repair verification** — Use `scripts/purge-stale-ptp-helpers.sh` (or `launchctl bootout system/com.audiout.Audiout.ptphelper`) to kill the helper's launchd job while the SMAppService record stays enabled, then relaunch the app. Confirm: (a) `ptp_zombie_heal` telemetry appears in logs, (b) speakers work again without manual Login Items toggle, (c) if repair reaches `.requiresApproval`, the setup screen flags .permissionLost([.ptpHelper]). The script serves dual purpose — reproduces the bug and clears stale daemons from concurrent dev builds. **2026-08-06 additions (the heal-loop regression):** (d) after a healed launch, quit and relaunch WITHOUT creating a new zombie — the relaunch must log NO `ptp_zombie_heal` (pre-fix: every launch healed); (e) after a heal, keep the app running 60s — `launchctl print system/<label>` must still find the job (pre-fix: a delayed BTM removal deleted it 12–30s post-heal, SIGTERMing the running helper); (f) the healed launch's telemetry carries `drain_polls` — expect a small number, and `heal_failed` with `drain_polls` means the teardown never drained (new, deliberate outcome instead of a doomed registration).
...
- **Test suites (combined tree):**
  ...
  - AudioutCore: 1529 tests passed (91 suites)

- **App build:** `scripts/make-app.sh` completed successfully.

- **LaunchDaemons plist** (`plutil -lint` and structure):
  - Validity: PASS
  - `MachServices` present: YES (`com.audiout.Audiout.ptphelper`)
  - `EnvironmentVariables.AUDIOUT_PTP_MACH_SERVICE`: present
  - `KeepAlive`: `{SuccessfulExit: false}` (correct)
  - `RunAtLoad` key: ABSENT (correct)
  - `Label`: `com.audiout.Audiout.ptphelper` (correct)
