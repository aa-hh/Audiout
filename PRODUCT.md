...
1. **Anonymous feature telemetry — opt-in, off by default.** Asked once, never re-nagged (the last card of first-run Setup). Answers feature usage, feature discoverability, uninstall rate, and month-over-month users.
   ...
   **Connection events follow what the user selected** (2026-09-10). `connection:failed` and `connection:connected` are captured only for a speaker the user wants audio on, so a speaker nobody selected losing its Bonjour advert no longer produces an event. The PostHog SDK's feature-flag preload is switched off in the same change (`preloadFeatureFlags = false`), so an install that declined tracking makes no request at launch; the SDK made one regardless of the opt-out until now.
