...
- **Hue-Neutral Warm Grey** (`#8D7D5E` dark / `#8B7958` light): the connected ring, and the dashed connecting ring. Deliberately not gold — connection is not the same as live, and giving connection the accent would spend the system's scarcest signal on its most common state. The light value was deepened on 2026-08-12 from `#A08C66`, which cleared 3:1 on the Mac's flat `panel` but failed on every darker light ground it is also drawn over (2.61:1 on Mac `well`, 2.71:1 on the iOS `well`). It now measures 3.37:1 at worst across all seven light grounds on both platforms.
...
**The Reserved-Bands Rule.** Two hue windows are reserved: the gold/amber window `[28°, 68°)`, because a colour there misreads as "armed"; and the failure-red window `[0°, 12°) ∪ [350°, 360°)`. The rule binds **derived and identity hues** — runtime per-app tether tints, which steer out of both, and the four permission hues, which sit ≥47° apart from each other and from both bands. It is declared in `AppTetherColor.ReservedBand`, alongside the code it governs.

It does **not** bind the authored warm-grey instruments. `ringConnected` sits at ~39°, inside the gold band, and always has — as does `gold` itself at ~41.5°. What separates the ring from the accent is **chroma, not hue**: the ring is a desaturated warm grey where gold is a saturated brass, and at the same hue that reads as two plainly different materials. Steering the ring out of the band would change a colour the system deliberately chose, to satisfy a rule written for a different problem.
...
**The direction is one flat white ground on both platforms** (Alec, 2026-08-12): *"The white, the flat light ground is what we want. We need to figure out a way to have contrast."*

Four mechanisms were built, rendered and measured against that brief — brief and light-mode renders at `dev/notes/light-separation-options-2026-08-12.md` on `claude/light-separation-options`. The measurements say the flat ground is not merely a preference to accommodate: **every ink and instrument is at its best on the near-white ground and gets worse on every rung down.** The iOS ladder buys 1.08:1 of separation at the cost of 0.4 of a contrast point on `label2` — and its steps (1.04–1.08:1) do not clear the project's own 1.10:1 surface floor anyway.

**Decided (Alec, 2026-08-12): separation moves from FILL to EDGE WEIGHT, on both platforms.** One mechanism, the hairline, at two weights — a stronger `#C4C0B4` (1.76:1 vs panel) for a container's own outer edge, today's `#D0CDC3` (1.54:1) for dividers inside it. `well` and the grouped-inset geometry are unchanged, and iOS retires its stepped light ladder so both platforms share the flat near-white ground.

Why this one won, and the property to protect: **nothing in the app is ever drawn *on* a hairline** — it is only ever a stroke or a divider fill — so moving it costs **zero** text contrast and **zero** instrument contrast. Every alternative paid somewhere. Deepening `well` drops `gold`, `ember`, `caution` and `faderThumb` under 3:1 *and* kills the container border it was meant to help, because pushing a container away from the page pushes it toward the ink sitting on it. A shadow registers at 1.35:1 but its darkest pixel is neutral grey where the hairline's is warm greige — a smudge on warm paper. Inset geometry alone measures 1.000:1: with flat fills and no edge there is no boundary pixel to measure.
...
**Resolved 2026-08-12, and it teaches the general lesson:** `ringConnected` / `ring` `#A08C66` passed on the Mac's flat light `panel` and failed on every darker light ground it is also drawn over. It is now `#8B7958`, chosen to clear ≥3:1 against **all seven** candidate light grounds across both platforms at once — including whatever the separation work above settles on — so a ground change cannot silently re-break it. Worst case 3.37:1, on Mac `well`.

The lesson, because this token failed the same way twice: **a dark instrument's binding constraint is the DARKEST light ground it is drawn over, never the whitest.** Contrast falls as the ground darkens toward the ink. Measuring an instrument against `panel` alone — the lightest, most obvious surface — is what let this through, and it is the same mistake that put light `gold` and `ember` under floor on `well` earlier the same day.
