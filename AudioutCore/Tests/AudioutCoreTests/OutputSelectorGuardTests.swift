...
@Suite final class OutputSelectorGuardTests: IsolatedSuite {
    ...
    @Test func defaultSystemOutputDeviceSelectorNeverUsedInRealCode() throws {
        ...
        // Walk up from the test file to find AudioutCore root:
        // #filePath = /path/to/.../AudioutCore/Tests/AudioutCoreTests/OutputSelectorGuardTests.swift
        // Target: /path/to/.../AudioutCore/Sources
        let fileURL = testFilePath
        let audioutCoreRoot = fileURL
            .deletingLastPathComponent()  // OutputSelectorGuardTests.swift → AudioutCoreTests
            .deletingLastPathComponent()  // AudioutCoreTests → Tests
            .deletingLastPathComponent()  // Tests → AudioutCore

        let audioutCoreSources = audioutCoreRoot.appendingPathComponent("Sources", isDirectory: true)

        // Also check AirPlayEngine/Sources if it's a sibling package
        let parentDir = audioutCoreRoot.deletingLastPathComponent()
        ...
        for sourceDir in [audioutCoreSources, airPlayEngineSources] {
