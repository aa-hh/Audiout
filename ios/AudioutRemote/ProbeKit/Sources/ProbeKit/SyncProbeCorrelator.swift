...
//
// ═══ THIS FILE IS A HAND-COPY. Change one, change the other. ═══
//
// The original is `AudioutCore/Sources/AudioutCore/SyncProbeCorrelator.swift`,
// where the Mac's own built-in-mic calibration uses it. `ios/` may never depend
// on AudioutCore (see `ios/AGENTS.md`), so the phone carries its own copy — the only
// file this package hand-copies (`ProbeAnalyzer.sweepSeconds` copies one
// constant from the Mac too; that is the whole of the rest, so do not go
// looking for a third). It is copied VERBATIM apart from this note: the Mac
// stages the sweeps this file describes, so a divergence here is not a local
// bug, it is a phone that measures the wrong signal and reports a confident
// wrong number.
//
// The duplication is a consequence of the package layout, not a preference. If
// AudioutCore is ever made to depend on ProbeKit instead, delete this copy —
// one file, one home, and the hazard goes with it.
