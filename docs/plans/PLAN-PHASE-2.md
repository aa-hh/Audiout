...
- **Q3 — Engine package placement & build model.** Where does the C+Swift engine
  ...
  - (a, **recommended**) A **new top-level SwiftPM package** `AirPlayEngine/`
    ...
    wrapping it; `AudioutCore` depends on it only via the
  ...
  - (b) Add a `CAirPlayEngine` target *inside* `AudioutCore/Package.swift`.
...
- files: `AudioutCore/Sources/AudioutCore/NativeBackend.swift`
  (new); `AudioutCore/Sources/AudioutCore/OwnToneBackend.swift`
  (edit `BackendKind` + `makeBackend`); `AudioutCore/Package.swift` (add
...
- files: delete `AudioutCore/Sources/AudioutCore/OwnToneBackend.swift`'s
...
- verify: `swift build` + `swift test` green with no `OwnTone`/`owntone` symbols
  remaining (`grep -ri owntone AudioutCore/Sources` empty); app still
...
- `AirPlayEngine/Sources/CAirPlayEngine/` (sources + shims) — **T-BUILD-1 and
  ...
  concurrently.**
- `AudioutCore/.../OwnToneBackend.swift` — T-BACKEND-1 AND T-CLEANUP-1
...
- `AudioutCore/.../BackendKindResolutionTests.swift` — T-BACKEND-1b after
  T-BACKEND-1.
