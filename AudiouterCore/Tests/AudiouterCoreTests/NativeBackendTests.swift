...
final class NativeBackendTests: XCTestCase {
    ...
    func testOrphanedCaptureAfterDeRouteIsStoppedNotAccepted() async {
        ...
        if case .idle = perAppCapture.state(for: "com.foo") {} else {
            XCTFail("an orphaned recapture (bundle ID no longer routed or local) must be " +
                     "stopped, not accepted — it stayed \(perAppCapture.state(for: "com.foo"))")
        }
        ...
        XCTAssertEqual(engine.streamAddCalls.filter { $0.0 == device.outputID }.count, bindCountAtDeRoute,
                       "an orphaned recapture must never cause a fresh per-app stream bind " +
                       "for the de-routed device")
    }
    ...
    func testDeviceToggleOffThenOnDoesNotTriggerSpuriousRecaptureReset() async {
        let perAppCapture = PerAppCaptureCoordinator(
        ...
        let binds = engine.streamAddCalls.filter { $0.0 == device.outputID }
        XCTAssertEqual(binds.count, 2,
                       "toggling a route off then back on is exactly two NATURAL binds " +
                       "(initial route + re-route) — a stale isRecapture must not fire an extra " +
                       "resetAirPlaySessionForRoutedApp on top, got \(binds.count)")
    }
    ...
    /// for them.)
    func testAppTerminatedClearsEverCapturedBundleIDs() async {
        let perAppCapture = PerAppCaptureCoordinator(
            makeTap: { AlwaysSucceedsTap() }, processResolver: singleProcessResolver(["com.foo": 4242]), muteBehavior: .mutedWhenTapped)
        let engine = SpyEngine()
        let discovery = FakeDiscovery()
        ...
        XCTAssertTrue(backend.test_hasEverCaptured(bundleID: "com.foo"),
                      "setup failed: the first capture must record the ever-captured bit")
        ...
        XCTAssertFalse(backend.test_hasEverCaptured(bundleID: "com.foo"),
                       "a quit must forget everCapturedBundleIDs for the terminated bundle, " +
                       "per its own doc comment (\"cleared ... when its capture stops\")")
    }
    ...
    func testCurrentDeviceAppTerminatedStopsItsCapture() async {
        ...
        if case .capturing = perAppCapture.state(for: "com.local") {} else {
            XCTFail("setup failed: capture never reached .capturing before handleAppTerminated")
            return
        }
        ...
        if case .idle = perAppCapture.state(for: "com.local") {} else {
            XCTFail("a `.currentDevice` app's quit must stop its per-app capture tap — " +
                     "it stayed \(perAppCapture.state(for: "com.local"))")
        }
    }
    ...
    func testCurrentDeviceAppRelaunchRestartsItsCapture() async {
        let perAppCapture = workingPerAppCapture(bundleIDs: ["com.local"])
        let (backend, engine, _) = makeBackend(injectedPerAppCapture: perAppCapture)
        backend.start(); defer { backend.stop() }
        await waitUntilStarted(engine)
        ...
        if case .capturing = perAppCapture.state(for: "com.local") {} else {
            XCTFail("a relaunched `.currentDevice` app must have its per-app capture tap " +
                     "restarted — it stayed \(perAppCapture.state(for: "com.local"))")
        }
    }
    ...
    // MARK: Permission-grant capture resume (T6-rev)

    /// A `ProcessAudioTap` that refuses the FIRST `createAndStart` per bundle id
    /// with the exact system-audio permission refusal
    /// (`PerAppCaptureError.notAuthorizedReason`), then succeeds on every later
    /// attempt — the shape of a real mid-session grant: the tap the app opened
    /// before the user granted was refused, and the identical call afterwards
    /// works. Records every attempt so a test can prove a `start` was actually
    /// re-issued rather than silently skipped.
    private final class RefusesFirstAttemptTap: ProcessAudioTap, @unchecked Sendable {
        /// Shared across every tap instance the coordinator builds, since it
        /// makes a NEW tap per attempt — per-instance state would forget that
        /// the first attempt already happened.
        final class Ledger: @unchecked Sendable {
            private let lock = NSLock()
            private var attempts: [String: Int] = [:]
            /// Records an attempt and reports whether it must be refused (the
            /// first one for this bundle id).
            func recordAndShouldRefuse(_ bundleID: String) -> Bool {
                lock.withLock {
                    let next = (attempts[bundleID] ?? 0) + 1
                    attempts[bundleID] = next
                    return next == 1
                }
            }
            func attemptCount(_ bundleID: String) -> Int { lock.withLock { attempts[bundleID] ?? 0 } }
        }
        ...
        func createAndStart(processes: Set<AudioProcess>, bundleID: String, muteBehavior: TapMuteBehavior) throws -> TapFormat {
            if ledger.recordAndShouldRefuse(bundleID) {
                throw PerAppCaptureError.tapCreationFailed(reason: PerAppCaptureError.notAuthorizedReason)
            }
            return TapFormat(sampleRate: 48000, channels: 2, bitsPerSample: 32, isFloat: true, isInterleaved: false)
        }
        func teardown() {}
    }

    /// THE case the ordinary route-change path misses: `updateAppRoutes` starts
    /// only `newUnion.subtracting(previousUnion)`, so re-pushing an UNCHANGED
    /// table yields an EMPTY start set and revives nothing. `resumeRefusedAppCaptures()`
    /// must start EVERY routed bundle regardless of the diff.
    func testResumeRefusedAppCapturesRestartsEveryRoutedBundleOnAnUnchangedTable() async {
        let ledger = RefusesFirstAttemptTap.Ledger()
        let perAppCapture = PerAppCaptureCoordinator(
            makeTap: { RefusesFirstAttemptTap(ledger: ledger) },
            processResolver: singleProcessResolver(["com.foo": 9001, "com.bar": 9002]),
            muteBehavior: .mutedWhenTapped,
            // No live HAL process-list listener: it fires on ANY process on the
            // machine opening an audio session and re-enters `start(bundleID:)`
            // from a HAL thread, which under `--parallel` load restarted the
            // refused tap on its own and made the attempt counts below racy.
        ...
        let routes = [
            AppRoute(bundleID: "com.foo", displayName: "Foo", destination: .device(id: device.id), volume: 100),
            AppRoute(bundleID: "com.bar", displayName: "Bar", destination: .device(id: device.id), volume: 100),
        ]
        backend.updateAppRoutes(routes)
        // Both taps are refused — the state a missing system-audio grant leaves.
        await pollUntil { ledger.attemptCount("com.foo") == 1 && ledger.attemptCount("com.bar") == 1 }
        await pollUntil {
            if case .failed = perAppCapture.state(for: "com.foo") { return true }
            return false
        }

        // Re-pushing the IDENTICAL table must change nothing — this is the trap.
        backend.updateAppRoutes(routes)
        try? await Task.sleep(nanoseconds: 200_000_000)
        XCTAssertEqual(ledger.attemptCount("com.foo"), 1,
                       "an unchanged route table produces an EMPTY captureToStart — no restart")
        XCTAssertEqual(ledger.attemptCount("com.bar"), 1)

        // The grant landing must revive both, with the route table untouched.
        backend.resumeRefusedAppCaptures()
        await pollUntil {
            if case .capturing = perAppCapture.state(for: "com.foo"),
               case .capturing = perAppCapture.state(for: "com.bar") { return true }
            return false
        }
        XCTAssertEqual(ledger.attemptCount("com.foo"), 2, "every routed bundle restarts, not just a diff")
        XCTAssertEqual(ledger.attemptCount("com.bar"), 2)
    }

    /// `.currentDevice` apps ride the SAME per-app tap as `.device` routes, so the
    /// resume set is the union of both — a local-routed app stranded by a refusal
    /// must come back too.
    func testResumeRefusedAppCapturesAlsoRevivesCurrentDeviceRoutes() async {
        let ledger = RefusesFirstAttemptTap.Ledger()
        let perAppCapture = PerAppCaptureCoordinator(
            makeTap: { RefusesFirstAttemptTap(ledger: ledger) },
            processResolver: singleProcessResolver(["com.local": 9003]),
            muteBehavior: .mutedWhenTapped,
            installsProcessListListener: false)   // see the sibling test above
        let (backend, engine, discovery) = makeBackend(injectedPerAppCapture: perAppCapture)
        await startAndDiscover(backend, engine, discovery, ap2Device())
        defer { backend.stop() }

        backend.updateAppRoutes([
            AppRoute(bundleID: "com.local", displayName: "Local", destination: .currentDevice, volume: 100),
        ])
        await pollUntil { ledger.attemptCount("com.local") == 1 }
        ...
        XCTAssertEqual(ledger.attemptCount("com.local"), 2)
    }

    /// `forceCaptureGateReevaluation()` must defeat the `want != captureRunning`
    /// short-circuit that strands the whole-system tap after a refused `start()`.
    /// The re-push of an identical selection is the control: it proves the
    /// short-circuit really is a no-op, so the extra `start` afterwards can only
    /// have come from the force.
    func testForceCaptureGateReevaluationClearsTheCaptureRunningShortCircuit() async {
        let (backend, engine, discovery) = makeBackend()
        let capture = FakeCapture()
        backend.captureCoordinator = capture
        defer { backend.stop() }
        let device = ap2Device(id: "AA:BB:CC:DD:EE:70", name: "Force Speaker")
        await connectAP2(backend, engine, discovery, device)
        await pollUntil { capture.isCapturing }
        XCTAssertEqual(capture.ops, ["start"])

        // Control: the selection hasn't changed, so the gate short-circuits and
        // a stranded (refused) coordinator would never be retried.
        backend.setOutputSet([device.id])
        try? await Task.sleep(nanoseconds: 200_000_000)
        XCTAssertEqual(capture.ops, ["start"], "an unchanged selection must not re-issue start")

        backend.forceCaptureGateReevaluation()
        await pollUntil { capture.ops.filter { $0 == "start" }.count == 2 }
        XCTAssertEqual(capture.ops, ["start", "start"],
                       "the force must re-issue start — and must never enqueue a stop")
    }

    /// The safety half: with NOTHING selected the gate wants capture OFF, and the
    /// force must be a complete no-op. It must never fabricate a `stop()` (nothing
    /// asked for one) and must never leave `captureRunning` claiming `false` while
    /// a live `.mutedWhenTapped` tap keeps the Mac's speakers silent — that is the
    /// failure mode that would mute the Mac permanently.
    func testForceCaptureGateReevaluationIsInertWhenNothingIsSelected() async {
        let (backend, engine, discovery) = makeBackend()
        let capture = FakeCapture()
        backend.captureCoordinator = capture
        defer { backend.stop() }
        let device = ap2Device(id: "AA:BB:CC:DD:EE:71", name: "Idle Speaker")
        await startAndDiscover(backend, engine, discovery, device)

        backend.forceCaptureGateReevaluation()
        try? await Task.sleep(nanoseconds: 200_000_000)
        XCTAssertTrue(capture.ops.isEmpty, "no selection ⇒ no capture op of any kind")

        // And the gate is still healthy afterwards: a real selection still starts.
        backend.setOutputSet([device.id])
        await pollUntil { capture.isCapturing }
        XCTAssertEqual(capture.ops, ["start"])
    }
    ...
    func testRoutingActionHookFiresFromBothChokepoints() async {
        let (backend, engine, discovery) = makeBackend()
        defer { backend.stop() }
        ...
        backend.setOutputSet([device.id])
        XCTAssertEqual(counter.count, 1, "setOutputSet is a routing action")

        backend.updateAppRoutes([
            AppRoute(bundleID: "com.foo", displayName: "Foo", destination: .noRedirect, volume: 100),
        ])
        XCTAssertEqual(counter.count, 2, "updateAppRoutes is a routing action")
    }
