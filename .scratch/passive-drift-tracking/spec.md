...
| 17 | (2026-09-13 proposal; SETTLED 2026-09-14 by merging ticket 13) No guessed correction is applied before a second window agrees within 1.5 ms (`DriftCorrectionPolicy.VerifyMode.verifyBeforeApply`, the init default). Apply-then-verify stays one line away. |
| 18 | (2026-09-14, Alec) **The microphone listens on events, not on a 3-minute clock.** Live tests 2–4 showed no continuous drift between speakers once the sender's dropped cycles were padded (PR #200); what remains is event-shaped: a reconnect or relaunch re-rolling a link by ~10 ms, a link going bad (clock steps), silence→audio. So windows run on: reconnect, a clock step (rate-limited per speaker), silence→audio, the manual re-sync, and at most ONE periodic check every 20–30 minutes. Decision 2's "short periodic windows" is amended accordingly; the mic light becomes a few-times-a-session event. Ticket 17. |
