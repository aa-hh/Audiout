...
**5. Nothing committed** — record `BASE=$(git -C "$WT" rev-parse HEAD)` as your very first action, before editing anything. At the end, `git -C "$WT" rev-parse HEAD` must still equal `$BASE`. Paste both, plus `git -C "$WT" status --short`, which should list only the files Tracks A and C touched, all unstaged. (Deliberately not a hardcoded SHA — this branch already carries docs commits, and naming one here goes stale the moment another lands.)
