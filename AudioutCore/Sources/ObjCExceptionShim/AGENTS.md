...
gap. It has no AudioutCore- or AVFAudio-specific knowledge; it is a single
block-based catcher.
...
- Swift code never calls `AUDCatchObjCException` directly. The sole call
  ...
  `AudioutCore/Sources/AudioutCore/ObjCExceptionCatching.swift`, which
...
None beyond `Foundation`. This target has no dependency on AudioutCore or
...
`AudioutCore/Tests/AudioutCoreTests/ObjCExceptionCatchingTests.swift`,
