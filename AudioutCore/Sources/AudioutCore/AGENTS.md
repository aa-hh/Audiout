...
- `BTAlignmentWizardSession` enters `.listening` only when its host's `requestListening` answers true; a rejected proposal never ends a run — only the wing rule, a stagnant belief, or the spent answer budget do.
