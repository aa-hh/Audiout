...
**OPEN RISK — volume on the aggregate (raised by live test 2026-07-26, gates final Wave 3 commitment).** With the aggregate as default output and Audiouter NOT running, **the volume keys do nothing** (Alec confirmed live). The device advertises volume capability (`aggtool status`: `vmvc=true scalarMain=true muteMain=true`) but does not deliver it — do not trust that flag. G2b's comparison accepted "no Sound-pane slider" on the grounds that the app's volume-key handling covers it while running; that assumption is now the load-bearing one and is **untested**. Before Wave 3 is finally committed to the aggregate, verify that the running app's volume-key path (volume keys drive Main Out — see the app's existing key handling) actually controls volume while the aggregate is the default output. If it does not, the aggregate costs the user volume control whenever the app isn't handling keys, and the HAL driver (which can implement a real volume control in ~80 lines, per G2) comes back into contention.
