...
2. **License activation check-ins — identified by purchase.** Ties a license ID to a device count. This is not anonymous and must never be described as such. **Not opt-in** (owner's call, 2026-08-24): unlike stream 1, this is abuse detection — a license appearing on far more devices than one buyer plausibly owns — so it cannot be a toggle an abuser simply switches off. It runs unconditionally whenever a key and a check-in endpoint exist, the same way any paid app's license check does; users are told about it (this document, the source), not asked to consent to it. A trial is identified the same way and for the same reason: the trial-start request carries a SHA-256 hash of the Mac's `IOPlatformUUID`, salted with a fixed public string. The raw hardware UUID never leaves the Mac, and the hash has exactly one job — refusing a second trial on the same machine. It is **not opt-in** either, and by the same reasoning: abuse prevention an abuser can switch off prevents nothing.
