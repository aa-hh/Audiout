...
///     `player.play/stop`, `configureOutputDevice`, touching `mainMixerNode`).
...
public final class LocalPlaybackEngine: LocalPlaybackControlling, @unchecked Sendable {
    ...
    /// `graphQueue` like `configuredDevice`. Cancelled again in `deinit`.
    ...
    private func startEngineOnGraphQueue() throws {
        ...
        if offlineRenderingForTests {
            ...
            // `configureOutputDevice` is skipped (its own offline guard also
            // no-ops it), so no hardware is ever touched.
