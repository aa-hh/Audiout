...
1. **Quit every other Audiout** (menu-bar icon → Quit; check `/Applications`, other worktrees).
...
actual leak proof for this wave is **Test 4** (real coreaudiod/Audiout RSS via `ps`) — treat
...
`~/Library/Logs/Audiout/telemetry.jsonl` (rotated to `.1` at 5 MB). After the device flip,
...
- **Phase C (2 min):** remove redirect, 30 s, **quit Audiout**, watch 2 more min. PASS = no big
