...
- **This Mac's own AirPlay receiver is never surfaced as a device** (Alec's
  ...
  target for ANOTHER Mac running Audiout), lift this at the discovery seam —
...
- **Any window/panel `show*()` entry point must gate its actual on-screen
  ...
  actually on screen. The real app (`AudioutApp`) never sets the env var and
...
- **Every run is `scripts/run-tests.sh`**, never a bare `swift test` — it
  ...
  - **A machine-wide concurrency CAP** (`AUDIOUT_TEST_SLOTS`, default **2**) —
    a counting semaphore over `/tmp/audiout-suite.lock.N`, shared across every
  ...
  - **Adaptive serial/parallel (`AUDIOUT_TEST_MODE=auto|parallel|serial`).**
  ...
  - **Remote Mac (opt-in, off by default).** Configure with `git config`, NOT an
    ...
    ```
    git config --local audiout.remoteHost 'user@192.168.4.41'
    git config --local audiout.testPrefer remote   # or: local (default), cpu
    git config --local audiout.testRemoteBias 40   # cpu mode only; see below
    ```
    ...
    reach some runs and not others. `AUDIOUT_TEST_REMOTE_HOST` /
    `AUDIOUT_TEST_PREFER` still override per-invocation.
    ...
    **These three settings govern BUILDS as well as tests.** The decision, the
    ...
    judged fine for a release build. The `AUDIOUT_TEST_` prefix is kept for
    compatibility with what is already configured; a parallel `AUDIOUT_BUILD_`
    ...
    `AUDIOUT_BUILD_LOCAL=1` forces a local compile for both `build.sh` and
    ...
    **A remote PASS is accepted; a remote FAILURE is re-run locally before it
    ...
    directory under `AUDIOUT_TEST_REMOTE_ROOT`. Cost is negligible: ~22MB/446
  ...
  Escape hatches: `AUDIOUT_TEST_MODE=parallel` (force the fast path when you
  are watching the terminal), `AUDIOUT_TEST_SLOTS=N` (concurrency cap,
  default 2), `AUDIOUT_TEST_NO_LOCK=1` (skip the cap entirely),
  `AUDIOUT_TEST_NO_CACHE=1` (force a real run), `AUDIOUT_TEST_WORKERS=N`.
  If all slots stay busy past `AUDIOUT_TEST_LOCK_TIMEOUT` (default 1800s)
  ...
  (current count via `git grep ': XCTestCase'` under `AudioutCore/Tests/`) —
...
- **Coverage gate (the one enforcement that matters):** `.githooks/pre-commit`
  ...
  files touch AudioutCore Swift sources/tests, and blocks the commit if it
...
- **Isolate shared state via `IsolatedSuite` (new/converted suites) or
  ...
  `Tests/AudioutCoreTests/IsolatedTestCase.swift` holds the shared
...
- **`Telemetry.log(...)` is always-on** (gated only by `HeadlessRuntime.isActive`,
  ...
  parent suite (`Tests/AudioutCoreTests/SerializedSharedStateSuite.swift`,
...
- **Tests must stay invisible — nothing a test does may reach the screen.**
  ...
  presenter. The `AudioutApp` executable (status item, activation policy) is
