...
- **D6 Radii.** Group card and New Group tile 10 → `Radius.row` (16; iOS Group Row "a `panel` card at row radius"). Card seat, member chip (6), picker cell (7), picker preview tile (6), `DeviceIconWellView` (12), member-row glyph tile (new), sidebar plate (8) → `Radius.control` (10; controls and chips). `GroupedSectionView.card` 10 → `Radius.panel` (26; iOS "panel radius for a grouped stack", /Users/alechenderson/Projects/audiout-remote/DESIGN.md:784-785); the new `.panel` fact-row style → `Radius.row` (16; iOS PanelRow, /Users/alechenderson/Projects/audiout-remote/DESIGN.md:900-902). The New Group tile's 28 pt plus-ring keeps `cornerRadius = 14` because it is a circle (28 / 2), same reading as PR 3's capsules. `Tokens.Layout.groupedSectionCornerRadius` stays for onboarding's `RoundedContainerView` (its only remaining consumer).
...
- iOS recipes: Group Row and glow /Users/alechenderson/Projects/audiout-remote/DESIGN.md:854-888, `GroupsView.swift:205-226` (glow: `RadialGradient` `partyRampDeep` at 0.22 dark / 0.10 light, `endRadius: 30`, 60×60 frame, hit-testing off, accessibility hidden); PanelRow /Users/alechenderson/Projects/audiout-remote/DESIGN.md:900-916; editor member row and picker /Users/alechenderson/Projects/audiout-remote/DESIGN.md:929-957, `GroupEditorView.swift:198-240`.
...
- Both sidebars on the bare `panel` ground with no material, dark and light (D1) — does the split divider alone carry the sidebar/pane split, or does Alec want the `NSVisualEffectView` sidebar material back?
...
- `dev/notes/window-snapshots/*.png` are stale (14 files); Alec's eye is the only verification of this window.
