...
final class WriteBacklogTests: XCTestCase {
    ...
    func testWritesUnderCapAllAdmit() {
        let guardUnit = WriteBacklogGuard(capSeconds: 1.0)
        let add: Additions = [(streamId: 1, audioSeconds: 0.1)] // 100ms/write

        for i in 1...9 {
            XCTAssertTrue(guardUnit.admit(add), "write \(i) under cap must admit")
            XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 1), Double(i) * 0.1, accuracy: 1e-9)
        }
        XCTAssertEqual(guardUnit.snapshot().droppedWrites, 0, "nothing under the cap should drop")
    }
    ...
    func testWriteAtCapIsDroppedAndObservable() {
        let guardUnit = WriteBacklogGuard(capSeconds: 0.1)
        let add: Additions = [(streamId: 1, audioSeconds: 0.05)] // 50ms/write

        XCTAssertTrue(guardUnit.admit(add))  // 50ms
        XCTAssertTrue(guardUnit.admit(add))  // 100ms == cap
        let atCap = guardUnit.inFlightSeconds(streamId: 1)

        XCTAssertFalse(guardUnit.admit(add), "a write for a stream at the cap must be dropped")
        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 1), atCap,
                       "a dropped write must not grow the backlog")
        XCTAssertEqual(guardUnit.snapshot().droppedWrites, 1, "the drop must be observable")
    }
    ...
    func testCapSelfHealsAfterDrain_notOneShot() {
        let guardUnit = WriteBacklogGuard(capSeconds: 0.1)
        let add: Additions = [(streamId: 1, audioSeconds: 0.05)]

        XCTAssertTrue(guardUnit.admit(add))   // 50ms
        XCTAssertTrue(guardUnit.admit(add))   // 100ms == cap
        XCTAssertFalse(guardUnit.admit(add))  // dropped
        XCTAssertFalse(guardUnit.admit(add))  // still dropping while at the ceiling
        XCTAssertEqual(guardUnit.snapshot().droppedWrites, 2)

        // A body drains (release) -> backlog falls below the cap.
        guardUnit.release(add)                // back to 50ms
        XCTAssertLessThan(guardUnit.inFlightSeconds(streamId: 1), guardUnit.capSeconds)
        ...
        XCTAssertTrue(guardUnit.admit(add), "after draining below the cap the stream must admit again")
        XCTAssertEqual(guardUnit.snapshot().droppedWrites, 2, "self-heal must not record a new drop")
    }
    ...
    func testOversizedSingleWriteNeverPermanentlyWedges() {
        let guardUnit = WriteBacklogGuard(capSeconds: 0.1)
        let big: Additions = [(streamId: 1, audioSeconds: 0.5)] // 5x the cap in one write

        XCTAssertTrue(guardUnit.admit(big), "an empty stream must admit even an oversized write")
        XCTAssertFalse(guardUnit.admit(big), "now over cap -> drop")
        guardUnit.release(big)                                   // drains
        XCTAssertTrue(guardUnit.admit(big), "must admit again once drained — never a permanent wedge")
    }
    ...
    func testPerStreamIsolation() {
        let guardUnit = WriteBacklogGuard(capSeconds: 0.1)
        let s1: Additions = [(streamId: 1, audioSeconds: 0.1)] // fills stream 1 to the cap
        let s2: Additions = [(streamId: 2, audioSeconds: 0.05)]

        XCTAssertTrue(guardUnit.admit(s1))    // stream 1 at cap
        XCTAssertFalse(guardUnit.admit(s1), "stream 1 is at the cap -> dropped")

        // Stream 2 is untouched by stream 1's congestion.
        XCTAssertTrue(guardUnit.admit(s2), "an unrelated stream must not be blocked by a stalled one")
        XCTAssertTrue(guardUnit.admit(s2))
        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 2), 0.1, accuracy: 1e-9)
    }
    ...
    func testBalancedAdmitReleaseReturnsToZeroAndPrunes() {
        let guardUnit = WriteBacklogGuard(capSeconds: 5.0)
        let add: Additions = [(streamId: 7, audioSeconds: 0.05)]

        for _ in 0..<10 { XCTAssertTrue(guardUnit.admit(add)) }
        XCTAssertEqual(guardUnit.snapshot().streamsTracked, 1)
        for _ in 0..<10 { guardUnit.release(add) }

        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 7), 0)
        XCTAssertEqual(guardUnit.snapshot().streamsTracked, 0, "a fully-drained stream must be pruned")
        XCTAssertEqual(guardUnit.snapshot().maxInFlightSeconds, 0)
    }
    ...
    func testBatchAdmittedOrDroppedAsAUnit() {
        let guardUnit = WriteBacklogGuard(capSeconds: 0.1)

        // Push stream 2 to the cap on its own.
        XCTAssertTrue(guardUnit.admit([(streamId: 2, audioSeconds: 0.1)]))
        ...
        XCTAssertFalse(guardUnit.admit(batch), "a batch with any at-cap stream is dropped as a unit")
        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 1), 0, "an under-cap member of a dropped batch is not charged")
        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 3), 0)

        // With stream 2 drained, the same batch now admits and charges every member.
        guardUnit.release([(streamId: 2, audioSeconds: 0.1)])
        XCTAssertTrue(guardUnit.admit(batch))
        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 1), 0.02, accuracy: 1e-9)
        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 3), 0.02, accuracy: 1e-9)
    }
    ...
    func testResetClearsEverything() {
        ...
        XCTAssertTrue(guardUnit.admit(add))
        XCTAssertFalse(guardUnit.admit(add)) // one drop recorded, stream at cap

        guardUnit.reset()

        let snap = guardUnit.snapshot()
        XCTAssertEqual(snap.droppedWrites, 0)
        XCTAssertEqual(snap.streamsTracked, 0)
        XCTAssertEqual(guardUnit.inFlightSeconds(streamId: 1), 0)
    ...
    func testEngineHealthyWritesAllLandWithNoDrops() async {
        ...
        XCTAssertEqual(airplay_test_master_session_input_buffer_samples(ams), UInt32(writes * samplesEach),
                       "every healthy write must reach the fan-out — the guard must not spuriously drop")
        let snap = engine.writeBacklogSnapshot()
        XCTAssertEqual(snap.droppedWrites, 0)
        XCTAssertEqual(engine.writeBacklog.inFlightSeconds(streamId: 5), 0,
    ...
    func testEngineDropsSaturatedStreamThenSelfHeals() async {
        ...
        XCTAssertTrue(engine.writeBacklog.admit(stream1Reservation))
        ...
        XCTAssertEqual(airplay_test_master_session_input_buffer_samples(ams2), 40,
                       "an unrelated stream must keep flowing while stream 1 is saturated")
        XCTAssertGreaterThanOrEqual(engine.writeBacklogSnapshot().droppedWrites, 1,
        ...
        XCTAssertEqual(airplay_test_master_session_input_buffer_samples(ams1), 100,
