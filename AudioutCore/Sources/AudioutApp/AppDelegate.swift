...
import AppKit
import AudioutCore
// Companion wire constants (`CompanionGoodbyeReason`) — the command cases
// already arrive transitively via AudioutCore's server/dispatcher API.
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Wave 3 T5 (routing-blocked warning's "Use Audiout" button, Q6): the
        ...
        // discipline (AudioutCore/AGENTS.md) at the per-app level. Called
    ...
    private func presentSetup(reason: OnboardingReason = .firstRun, model providedModel: SetupModel? = nil) {
        ...
        let controller = OnboardingWindowController(model: model, reason: reason) { [weak self] in
            ...
            // (2026-07-25, see `AudioutCore/AGENTS.md`); relaunching after the
    ...
    private func installMainMenu() {
        ...
        let quit = appMenu.addItem(withTitle: "Quit Audiout", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        quit.target = NSApp
