...
@Suite struct MainOutRowMenuDispatchTests {
    ...
    /// group under an "Output Scenes" header. `current` selects the checkmark.
        ...
        -> (MainOutRowView, RecordingDelegate)
    {
        ...
        row.apply(options: [
            ...
            .init(title: "Output Scenes", isHeader: true),
    ...
    @Test func outputGroupsHeaderIsDisabledAndInert() {
        ...
        let header = row.test_menuItem(titled: "Output Scenes")
        #expect(header != nil, "the 'Output Scenes' section header must exist")
